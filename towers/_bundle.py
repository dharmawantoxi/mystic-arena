...
class _NS_cannon_tower:
    ...
    def _draw_cannon(canvas, cx, cy, face, palette, tier, scale=1.0,
                     alt_timing=False, tower=None):
        """Main cannon body with barrel"""

        # ═══ WOODEN CARRIAGE (base under cannon) ═══
        carr_w = int(20 * scale)
        carr_h = int(8 * scale)
        carr_y = cy + 4
        ...
        pygame.draw.rect(canvas, palette['wood_dark'],
                         ...
                         border_radius=1)
        pygame.draw.rect(canvas, palette['wood_mid'],
                         (cx - carr_w // 2, carr_y, carr_w - 2, 4))
        pygame.draw.rect(canvas, palette['wood_light'],
                         (cx - carr_w // 2, carr_y, carr_w - 3, 2))
        pygame.draw.rect(canvas, palette['wood_high'],
                         (cx - carr_w // 2 + 1, carr_y + 1, 3, 1))

        # Gold reinforcements on carriage
        if tier in ('bronze', 'iron', 'legendary'):
            # Corner brackets
            for corner_x in [-carr_w // 2, carr_w // 2 - 3]:
                pygame.draw.rect(canvas, palette['gold_dark'],
                ...
                pygame.draw.rect(canvas, palette['gold_mid'],
                                 (cx + corner_x, carr_y, 2, carr_h - 1))
                pygame.draw.rect(canvas, palette['gold_light'],
                                 (cx + corner_x, carr_y, 1, 3))
                pygame.draw.rect(canvas, palette['gold_high'],
                                 (cx + corner_x, carr_y, 1, 1))
                # Rivet
                _NS_cannon_tower._aacircle(canvas, palette['iron_darkest'],
                          (cx + corner_x + 1, carr_y + carr_h - 2), 1)
                pygame.draw.rect(canvas, palette['iron_high'],
                                 (cx + corner_x + 1,
                                  carr_y + carr_h - 2 - 1, 1, 1))

        # ═══ CANNON PIVOT (metal cylinder for rotation) ═══
        pivot_y = cy + 2
        _NS_cannon_tower._aacircle(canvas, palette['shadow_deep'], (cx + 1, pivot_y + 1), 3)
        _NS_cannon_tower._aacircle(canvas, palette['iron_darkest'], (cx, pivot_y), 3)
        _NS_cannon_tower._aacircle(canvas, palette['iron_dark'], (cx, pivot_y), 3)
        _NS_cannon_tower._aacircle(canvas, palette['iron_mid'], (cx - 1, pivot_y - 1), 2)
        _NS_cannon_tower._aacircle(canvas, palette['iron_light'], (cx - 1, pivot_y - 1), 1)

        # Gold decoration on pivot (side plates)
        for side in [-1, 1]:
            px = cx + side * 4
            pygame.draw.rect(canvas, palette['gold_dark'],
                             (px - 1, pivot_y - 1, 2, 3))
        ...
        # ═══ CANNON BARREL (main body - horizontal) ═══
        if tier == 'legendary':
            barrel_len = int(16 * scale)
            barrel_h = int(9 * scale)
        elif tier in ('bronze', 'iron'):
            barrel_len = int(15 * scale)
            barrel_h = int(8 * scale)
        else:
            barrel_len = int(13 * scale)
            barrel_h = int(7 * scale)

        barrel_y = cy - 3

        # Barrel points toward face direction
        # Back (fat end at pivot)
        back_x = cx - int(barrel_len * 0.3) * face
        front_x = cx + barrel_len * face

        # Draw barrel as horizontal shape (cylinder)
        # Body rectangle
        barrel_top = barrel_y - barrel_h // 2
        barrel_bot = barrel_y + barrel_h // 2
        ...
        # Shadow
        pygame.draw.rect(canvas, palette['shadow_deep'],
                         (bx_start + 1, barrel_top + 1,
                          bx_end - bx_start, barrel_h),
                         border_radius=3)

        # Iron layers (cylinder shading)
        pygame.draw.rect(canvas, palette['iron_darkest'],
                         (bx_start, barrel_top,
                          bx_end - bx_start, barrel_h),
                         border_radius=3)
        pygame.draw.rect(canvas, palette['iron_dark'],
                         (bx_start, barrel_top,
                          bx_end - bx_start - 1, barrel_h - 1),
                         border_radius=3)
        pygame.draw.rect(canvas, palette['iron_mid'],
                         (bx_start, barrel_top + 1,
                          bx_end - bx_start - 2, barrel_h - 3),
                         border_radius=2)
        # Top highlight (curved cylinder)
        pygame.draw.rect(canvas, palette['iron_light'],
                         (bx_start + 1, barrel_top + 1,
                          bx_end - bx_start - 3, 2),
                         border_radius=1)
        pygame.draw.rect(canvas, palette['iron_high'],
        ...
        # ═══ GOLD BANDS on barrel (match reference) ═══
        if tier == 'basic':
            band_positions = [0.3]  # 1 band
        elif tier == 'bronze':
            band_positions = [0.25, 0.55]  # 2 bands
        elif tier in ('iron', 'legendary'):
            band_positions = [0.2, 0.5, 0.75]  # 3 bands
        else:
            band_positions = [0.3, 0.6]

        for band_t in band_positions:
            band_x = int(bx_start + (bx_end - bx_start) * band_t)
            # Gold band
            pygame.draw.rect(canvas, palette['gold_darkest'],
                             (band_x - 1, barrel_top - 1, 3, barrel_h + 2))
            pygame.draw.rect(canvas, palette['gold_dark'],
                             (band_x - 1, barrel_top - 1, 3, barrel_h + 1))
            pygame.draw.rect(canvas, palette['gold_mid'],
                             (band_x - 1, barrel_top, 2, barrel_h))
            pygame.draw.rect(canvas, palette['gold_light'],
                             (band_x - 1, barrel_top + 1, 1, barrel_h // 2))
            pygame.draw.rect(canvas, palette['gold_high'],
                             (band_x - 1, barrel_top + 1, 1, 2))

        # Extra gold band at muzzle (front rim)
        muzzle_band_x = front_x - 2 * face
        for offset in range(3):
            bx_line = muzzle_band_x + offset * face
            pygame.draw.line(canvas, palette['gold_dark'],
                             (bx_line, barrel_top),
                             (bx_line, barrel_bot - 1), 1)
        pygame.draw.line(canvas, palette['gold_mid'],
                         (muzzle_band_x, barrel_top + 1),
                         (muzzle_band_x, barrel_bot - 2), 1)
        pygame.draw.line(canvas, palette['gold_light'],
                         (muzzle_band_x, barrel_top + 1),
                         (muzzle_band_x, barrel_top + 3), 1)

        # ═══ MUZZLE (dark hole at front - match reference) ═══
        muzzle_x = front_x
        muzzle_y = barrel_y
        muzzle_r = max(2, barrel_h // 2 - 1)
        ...
        _NS_cannon_tower._aacircle(canvas, palette['iron_darkest'],
                  (muzzle_x - 1 * face, muzzle_y), muzzle_r)
        # Inner darkness
        _NS_cannon_tower._aacircle(canvas, (5, 5, 8),
                  (muzzle_x - 1 * face, muzzle_y), muzzle_r - 1)

        # Muzzle rim highlight
        pygame.draw.rect(canvas, palette['iron_high'],
                         (muzzle_x - 1 * face, muzzle_y - muzzle_r + 1, 1, 1))
        pygame.draw.rect(canvas, palette['shine'],
                         (muzzle_x - 2 * face, muzzle_y - muzzle_r + 1, 1, 1))

        # ═══ BACK END DECORATION ═══
        # Small metal knob at back
        back_end_x = bx_start + (2 if face > 0 else -2)
        _NS_cannon_tower._aacircle(canvas, palette['iron_darkest'],
                  (back_end_x, barrel_y), 2)
        _NS_cannon_tower._aacircle(canvas, palette['iron_mid'], (back_end_x, barrel_y), 1)
        pygame.draw.rect(canvas, palette['iron_high'],
                         (back_end_x, barrel_y - 1, 1, 1))

        # ═══ LEGENDARY EXTRA (ornate details) ═══
        if tier == 'legendary':
            # Extra ornate ring around barrel
            for extra_t in [0.4, 0.85]:
                ex = int(bx_start + (bx_end - bx_start) * extra_t)
                pygame.draw.rect(canvas, palette['gold_high'],
                                 (ex, barrel_top - 1, 1, barrel_h + 2))
            # Glowing gem on top
            pulse = math.sin((tower.timer if tower else 0) * 0.1) * 0.3 + 0.7
            gem_x = cx + int(barrel_len * 0.35) * face
            gem_y = barrel_top - 1
            for glow_r in range(4, 0, -1):
                alpha = int(120 * pulse - glow_r * 20)
                if alpha > 0:
                    glow = pygame.Surface((10, 10), pygame.SRCALPHA)
                    _NS_cannon_tower._aacircle(glow, (255, 200, 50, alpha), (5, 5), glow_r)
                    canvas.blit(glow, (gem_x - 5, gem_y - 5))
            _NS_cannon_tower._aacircle(canvas, palette['gold_darkest'], (gem_x, gem_y), 2)
            _NS_cannon_tower._aacircle(canvas, (255, 220, 100), (gem_x, gem_y), 1)
            pygame.draw.rect(canvas, palette['shine'],
                             (gem_x, gem_y, 1, 1))
...
class _NS_ice_tower:
    ...
    def _draw_crystal_group(canvas, cx, cy, palette, timer, size='small',
                            offset=0, tower=None):
        """Cluster of crystals (multiple spikes together)"""
        pulse = math.sin(timer * 0.08 + offset) * 0.3 + 0.7

        if size == 'small':
            # 3 crystals (1 center tall + 2 side)
            crystals = [
                (0, 0, 14, 0),  # center tall
                (-4, 2, 8, 0.3),  # left short
                (4, 2, 8, 0.5),  # right short
            ]
        elif size == 'medium':
            # 5 crystals
            crystals = [
                (0, -2, 16, 0),  # center tallest
                (-5, 1, 10, 0.3),  # left mid
                (5, 1, 10, 0.5),  # right mid
                (-3, 4, 6, 0.8),  # left short
                (3, 4, 6, 1.0),  # right short
            ]
        elif size == 'legendary':
            # 7+ MASSIVE crystals
            crystals = [
                (0, -6, 24, 0),  # HUGE center
                (-6, -1, 14, 0.3),  # left tall
                (6, -1, 14, 0.5),  # right tall
                (-9, 3, 10, 0.7),  # far left
                (9, 3, 10, 0.9),  # far right
                (-3, 4, 8, 1.1),  # left front
                (3, 4, 8, 1.3),  # right front
                (0, 5, 6, 1.5),  # front center
            ]
        else:
            crystals = [(0, 0, 12, 0)]

        # ═══ BIG AURA GLOW (behind all crystals) ═══
        aura_size = 20 if size == 'legendary' else 14
        aura = pygame.Surface((aura_size * 3, aura_size * 3), pygame.SRCALPHA)
        for r in range(aura_size, 3, -2):
            alpha = int((aura_size - r) * 4 * pulse)
            if alpha > 0:
                _NS_ice_tower._aacircle(aura, (*palette['glow_mid'], alpha),
        ...
        # ═══ DRAW CRYSTALS (back to front - by y position) ═══
        # Sort by size to draw back crystals first
        sorted_crystals = sorted(crystals,
                                 key=lambda c: c[2], reverse=True)

        for cx_off, cy_off, height, pulse_off in sorted_crystals:
            _NS_ice_tower._draw_crystal_spike_detailed(canvas, cx + cx_off, cy + cy_off,
                                         height, palette, timer,
                                         pulse_offset=pulse_off)

        # ═══ LEGENDARY: floating shards around ═══
        if size == 'legendary' and tower:
            _NS_ice_tower._draw_floating_shards(canvas, cx, cy, palette, tower.timer)


    def _draw_crystal_spike_detailed(canvas, cx, cy, height, palette, timer,
                                     pulse_offset=0):
        """Detailed crystal spike (faceted)"""
        pulse = math.sin(timer * 0.1 + pulse_offset) * 0.2 + 0.8
        width = max(3, height // 3)

        # Individual glow
        glow_r = width + 2
        glow = pygame.Surface((glow_r * 2, height + glow_r * 2),
                              pygame.SRCALPHA)
        for r in range(glow_r, 0, -1):
            alpha = int((glow_r - r) * 8 * pulse)
            if alpha > 0:
                _NS_ice_tower._aacircle(glow, (*palette['glow_light'], alpha),
                          (glow_r, height // 2 + glow_r), r)
        canvas.blit(glow, (cx - glow_r, cy - height))
        ...
        _NS_ice_tower._aapoly(canvas, palette['crystal_high'], [
            (top_x, top_y + 3),
            (mid_l[0] + 2, mid_l[1] + 2),
            (bot_l[0] + 3, bot_l[1] - 2),
        ])

        # Center highlight line (down middle)
        pygame.draw.line(canvas, palette['crystal_shine'],
                         (top_x, top_y),
                         (cx, cy - 1), 1)
    ...
    def _draw_floating_shards(canvas, cx, cy, palette, timer):
        """Small floating crystal shards around legendary crystal"""
        for i in range(6):
            angle = timer * 0.03 + i * (math.pi * 2 / 6)
            radius = 22 + int(math.sin(timer * 0.05 + i) * 3)
            sx = cx + int(math.cos(angle) * radius)
            sy = cy + int(math.sin(angle) * 8) - 5
            ...
            # Trail glow
            glow = pygame.Surface((8, 8), pygame.SRCALPHA)
            _NS_ice_tower._aacircle(glow, (*palette['glow_mid'], 100), (4, 4), 3)
            canvas.blit(glow, (sx - 4, sy - 4))
