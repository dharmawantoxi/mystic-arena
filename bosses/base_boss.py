...
BOSS_LABEL_TOP = {
    ...
    "nazulmor": 73,
    "nexthyrius": 85,
    "nyxara": 98,   # rig rewrite v2: puncak tanduk mahkota ~94 px (diukur) + aman
    "nyxarath": 66,
    "nyxaroth": 62,
    "nixweaver": 55,
    "nyxallaria": 55,
...
class Boss(TowerDebuffMixin):
        ...
        if self.target:
            ...
            if dist <= self.range:
                if self.timer == 0:
                    ...
                    # Greatsword magma Ignis mendarat: flash bintang,
                    # sabit api, serpihan obsidian + bara, shake, dan
                    # hit-stop 0.03-0.08 s (paket lengkapnya di
                    # heroes/ignis_drachorn_fx.notify_melee_impact).
                    # Difilter per boss_type + try/except (paritas Gornak).
                    if getattr(self, 'boss_type', None) == 'ignis_drachorn':
                        try:
                            from heroes import ignis_drachorn_fx as _ignfx
                            if getattr(self, 'attack_range', 0) > 120:
                                _ignfx.notify_projectile_impact(
                                    self, self.target.x, self.target.y, 0.0,
                                    self.damage, False, 'orb')
                            else:
                                _ignfx.notify_melee_impact(
                    ...
                    if getattr(self, 'boss_type', None) == 'vhalzun':
                        try:
                            ...
                            pass
                    # ═══ IMPACT FX NYXARA ═══
                    # The Nether Matron bertarung dua wujud: ayunan
                    # tongkat tengkorak (jarak <= MELEE_REACH dunia
                    # 88 px) atau nether orb (jarak lebih jauh).
                    # Keduanya memicu flash nether, serpihan tulang,
                    # shockwave bergerigi, shake, dan hit-stop
                    # 0.03-0.08 s (lengkapnya di heroes/nyxara_fx).
                    # Difilter per boss_type + try/except (paritas
                    # Vhalzun).
                    if getattr(self, 'boss_type', None) == 'nyxara':
                        try:
                            from heroes import nyxara_fx as _nyxfx
                            if dist <= _nyxfx.MELEE_REACH:
                                _nyxfx.notify_melee_impact(
                                    self, self.target, self.damage, False)
                            else:
    ...
    def _cast_q_dragon_breath(self, enemies):
        ...
        # ═══ FX DRAGON BREATH (lapisan hidup) ═══
        try:
            from heroes import ignis_drachorn_fx as _ignfx
            _ignfx.notify_skill_cast(self, 'q')
            _ignfx.notify_skill_impact(self, self.target.x, self.target.y,
                                       max_range * 0.55, 'q')
        except Exception:
            pass
    ...
    def _cast_w_dragon_tail(self, enemies):
        ...
        # ═══ FX DRAGON TAIL (lapisan hidup) ═══
        try:
            from heroes import ignis_drachorn_fx as _ignfx
            _ignfx.notify_skill_cast(self, 'w')
            _ignfx.notify_skill_impact(self, self.x, self.y, 130, 'w')
        except Exception:
            pass
    ...
    def _cast_e_dragon_blood(self):
        ...
        # ═══ FX DRAGON BLOOD (lapisan hidup) ═══
        try:
            from heroes import ignis_drachorn_fx as _ignfx
            _ignfx.notify_skill_cast(self, 'e')
            _ignfx.notify_skill_impact(self, self.x, self.y, 110, 'e')
        except Exception:
            pass
    ...
    def _cast_r_elder_dragon_form(self, enemies):
        ...
        # ═══ FX ELDER DRAGON FORM (lapisan hidup) ═══
        try:
            from heroes import ignis_drachorn_fx as _ignfx
            _ignfx.notify_skill_cast(self, 'r')
            _ignfx.notify_skill_impact(self, self.x, self.y, 220, 'r')
        except Exception:
            pass
    ...
    def _smart_ai_nyxara(self, enemies, target_dist):
        ...
        if (
            ...
            # ═══ SKILL FX NYXARA (Life Drain: tether jiwa) ═══
            try:
                from heroes import nyxara_fx as _nyxfx
                _nyxfx.notify_skill_cast(self, 'r')
            except Exception:
                pass
        ...
        if nearby >= 3 and self.e_timer == 0:
            ...
            # ═══ SKILL FX NYXARA (Nether Ward: totem tengkorak) ═══
            try:
                from heroes import nyxara_fx as _nyxfx
                _nyxfx.notify_skill_cast(self, 'e')
                _nyxfx.notify_skill_impact(self, self.x, self.y, 100, 'e')
            except Exception:
                pass
        ...
        if (
            ...
            # ═══ SKILL FX NYXARA (Decrepify: kurva kutukan + sigil) ═══
            try:
                from heroes import nyxara_fx as _nyxfx
                _nyxfx.notify_skill_cast(self, 'w')
            except Exception:
                pass
        ...
        if self.target and self.q_timer == 0:
            ...
            # ═══ SKILL FX NYXARA (Nether Blast: nova bergerigi) ═══
            try:
                from heroes import nyxara_fx as _nyxfx
                _nyxfx.notify_skill_cast(self, 'q')
                _nyxfx.notify_skill_impact(
                    self, self.target.x, self.target.y, 130, 'q')
            except Exception:
                pass
    ...
    def take_damage(self, damage, from_team, damage_type='normal',
        ...
        if self.hp <= 0:
            ...
            # ═══ DEATH FX NYXARA ═══
            # Energi nether Matron meledak keluar: hit-stop 0.08 s,
            # shake, dan ledakan partikel wisp + serpihan tulang
            # (lengkapnya di heroes/nyxara_fx.notify_death). Difilter
            # per boss_type + try/except (paritas hook benturan).
            if getattr(self, 'boss_type', None) == 'nyxara':
                try:
                    from heroes import nyxara_fx as _nyxfx
