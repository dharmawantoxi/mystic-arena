...
sheet review-nya: tools/_shot_gornak_masterwork.py. Drakar tetap memakai
renderer aslinya (disukai) dengan polesan tipis. 
...
class _NS_drakar:
    ...
    def _draw_axe(surface, cx, cy, facing, angle, action, attack_progress,
        ...
        dx = math.cos(angle)
        dy = math.sin(angle)
        # Polesan: alur darah bilah menyala saat menyerang / skill aktif
        hot_blade = (action == "attack" or action == "cull"
                     or action == "rage" or attack_progress > 0)
        ...
        # Polesan: tepi putih 1 px di bilah bawah juga (dulu hanya bilah
        # atas yang punya - keduanya kini terbaca sama tajamnya)
        pygame.draw.line(surface, _NS_drakar.PALETTE["white"],
                         (bot_tip_x, bot_tip_y), (bot_mid1_x, bot_mid1_y), 1)

        # Polesan: alur darah tipis di kedua bilah - menyala saat
        # menyerang (hot), redup halus saat idle. Tema Red Mist, tanpa
        # mengubah siluet.
        groove_a = _NS_drakar._alpha(70 + 120 * (1 if hot_blade else 0))
        pygame.draw.line(surface,
                         _NS_drakar._rgba(_NS_drakar.PALETTE["blood_mid"],
                                          groove_a),
                         (int((head_cx + top_tip_x) / 2),
                          int((head_cy + top_tip_y) / 2)),
                         (int((head_cx * 0.75 + top_tip_x * 0.25)),
                          int((head_cy * 0.75 + top_tip_y * 0.25))), 1)
        pygame.draw.line(surface,
                         _NS_drakar._rgba(_NS_drakar.PALETTE["blood_mid"],
                                          groove_a),
                         (int((head_cx + bot_tip_x) / 2),
                          int((head_cy + bot_tip_y) / 2)),
                         (int((head_cx * 0.75 + bot_tip_x * 0.25)),
                          int((head_cy * 0.75 + bot_tip_y * 0.25))), 1)
