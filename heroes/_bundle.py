...
class _NS_grimjaw:
    ...
    def _draw_grimjaw_elite(surface, cx, cy, facing, phase, action,
                            ...
                            crit=False, omni=False):
        """Rig Grimjaw v3 - berserker bersih, ditulis ulang dari nol.

        Identitas dipertahankan (mask putih + mahkota api + flame scimitar),
        proporsi & kebersihan siluet dirombak ala pass Kaizen v5: dada
        ber-armor panel lebar, mask besar dengan dua celah mata menyala,
        mahkota api kompak, kaki chunky dengan greave, selout 1 px, tanpa
        hiasan 1 px yang hilang saat di-downscale ke arena.

        Semua koordinat lokal: (0,0) = jangkar pinggul, x maju (facing),
        y ke bawah. Telapak menapak di +70 (kontrak lama dipertahankan).
        Pose helper & timeline serang tidak berubah -> FX layer tetap
        sinkron dengan bilah.
        """
        ...
        # ==== 1. GERAK BADAN: root / lean / sway / crest lag ====
        crest_tilt = 0.0
        if walk:
            root_y = int(math.sin(phase * 2.0) * 2.5) - 2
            sway = int(math.sin(phase) * 3.0)
            lean = (4 + int(abs(stride) * 2)) * f
            crest_tilt = math.sin(phase + 2.6) * 0.06
        elif attack:
            pose = _NS_grimjaw._attack_pose(ap)
            root_y = pose["bob"]
            sway = 0
            lean = pose["lean"] * f
            t_rec = max(0.0, min(1.0,
                        (ap - _NS_grimjaw.ATTACK_SWING_END) /
                        (1.0 - _NS_grimjaw.ATTACK_SWING_END)))
            ...
            sway = 0
            lean = 0
            crest_tilt = math.sin(phase * 3.0) * 0.04
        else:
        ...
        if spin:
            flare = 1.10
        if fury:
            flare *= 1.10 + 0.04 * math.sin(phase * 3)
        if crit or omni:
            flare *= 1.06
        ...
        # ==== 3. LENGAN BELAKANG (tangan kosong di sisi tubuh) ====
        rear_shoulder = (-15, -34)
        if attack:
            # Pemulihan: lengan kembali PERSIS ke pose jaga di ap=1.0
            # (t_rec = 1 -> idle; loop closure tanpa pop antar frame).
            rec = max(0.0, min(1.0, t_rec))
            iebx, ieby = -19, -12 + int(breath)
            ihx, ihy = -21, -1 + int(breath)
            rear_elbow = (-20 + int((iebx + 20) * rec),
                          -12 + int((ieby + 12) * rec))
            rear_hand = (-22 + int((ihx + 22) * rec),
                         -2 + int((ihy + 2) * rec))
        elif walk:
            rear_elbow = (-19, -12 + int(stride * 5))
            rear_hand = (-22, -2 + int(stride * 6))
        elif spin:
            rear_elbow = (-21, -18)
            rear_hand = (-23, -26)
        else:
            rear_elbow = (-19, -12 + int(breath))
            rear_hand = (-21, -1 + int(breath))
        ...
        poly(p["skin_darkest"],
             [(rear_hand[0] - 4, rear_hand[1] - 3),
              (rear_hand[0] + 4, rear_hand[1] - 3),
              (rear_hand[0] + 4, rear_hand[1] + 3),
              (rear_hand[0] - 4, rear_hand[1] + 3)], False)
        _NS_grimjaw._aacircle(surface, p["skin_high"],
                              (rhx - f, rhy - 2), 2, False)
        for kn in (-2, 0, 2):
            _NS_grimjaw._rect(surface, p["skin_light"],
                              (int(rhx + f * 2) + kn - 1, int(rhy) + 1,
                               2, 2))

        # ==== 4. KAKI: paha / lutut / greave / boot chunky (foot solver) ====
        ...
        for side, step, lift, shn in ((-1, rear_step, rear_lift,
                                       p["metal_dark"]),
                                      (1, front_step, front_lift,
                                       p["metal_mid"])):
            hipx = side * 12
            tx = hipx + int(step * 0.3)
            kx = side * 13 + int(step * 0.5)
            fx = side * 13 + step
            # paha armor kulit: kolom terpisah tegas (gap terlihat)
            poly(p["armor_darkest"], [(tx - 6, 9), (tx + 6, 9),
                 (kx + 5, 30 - lift), (kx - 5, 30 - lift)])
            poly(p["armor_dark"], [(tx - 5, 10), (tx + 5, 10),
                 (kx + 4, 29 - lift), (kx - 4, 29 - lift)], False)
            # pelindung lutut emas + greave baja (kolom 12 px)
            poly(p["gold_darkest"], [(kx - 6, 28 - lift),
                 (kx + 6, 28 - lift), (kx + 5, 34 - lift),
                 (kx - 5, 34 - lift)], False)
            poly(p["gold_mid"], [(kx - 4, 29 - lift), (kx + 4, 29 - lift),
                 (kx + 3, 33 - lift), (kx - 3, 33 - lift)], False)
            poly(p["metal_darkest"], [(kx - 7, 34 - lift),
                 (kx + 7, 34 - lift), (kx + 6, 53 - lift),
                 (kx - 6, 53 - lift)], False)
            poly(p["metal_dark"], [(kx - 6, 35 - lift), (kx + 6, 35 - lift),
                 (kx + 5, 52 - lift), (kx - 5, 52 - lift)], False)
            poly(shn, [(kx - 4, 36 - lift), (kx + 4, 36 - lift),
                 (kx + 4, 51 - lift), (kx - 4, 51 - lift)], False)
            _NS_grimjaw._aaline(surface, p["metal_light"],
                                pt(kx - f * 4, 37 - lift),
                                pt(kx - f * 4, 50 - lift), 1)
            # boot lebar sedang (bukan papan), sol jelas
            poly(p["boot_darkest"], [(fx - 8, 53 - lift), (fx + 7, 53 - lift),
                 (fx + 8, 67 - lift), (fx - 9, 67 - lift)], False)
            poly(p["boot_dark"], [(fx - 7, 54 - lift), (fx + 6, 54 - lift),
                 (fx + 7, 66 - lift), (fx - 8, 66 - lift)], False)
            poly(p["boot_mid"], [(fx - 5, 55 - lift), (fx + 4, 55 - lift),
                 (fx + 5, 64 - lift), (fx - 6, 64 - lift)], False)
            _NS_grimjaw._aaline(surface, p["boot_light"],
                                pt(fx - 5, 53 - lift), pt(fx - 2, 53 - lift),
                                1)
            # jari kaki ke arah hadap (+f), sol gelap tebal
            toe = 6 * f
            foot = pt(fx + (4 if f > 0 else -4), 67 - lift)
            _NS_grimjaw._aaline(surface, p["shadow_deep"],
                                (foot[0] - toe, foot[1] + 2),
                                (foot[0] + toe, foot[1] + 2), 5)
            _NS_grimjaw._aaline(surface, p["boot_darkest"],
                                (foot[0] - toe, foot[1]),
                                (foot[0] + toe, foot[1]), 2)
            # bayangan kontak per telapak saat menapak + debu api
            if walk and lift == 0 and abs(stride_vel) < 0.5:
                fx_, fy_ = pt(fx, 69)
                _NS_grimjaw._ellipse(surface, (*p["shadow_deep"], 70),
                                     (fx_ - 12, fy_ - 2, 24, 7))
                if abs(stride_vel) < 0.3:
                    for k in range(3):
                        _NS_grimjaw._aacircle(
                            surface, (*p["fire_mid"], max(20, 110 - k * 30)),
                            (fx_ - k * 5 * f, fy_ - 2 - k * 2),
                            max(1, 3 - k))
        # ==== 5. TORSO: cuirass panel lebar + sash merah ====
        poly(p["armor_darkest"], [(-16, -38), (16, -38), (19, -32),
             (17, -20), (14, -8), (12, 7), (-12, 7), (-14, -8),
             (-17, -20), (-19, -32)])
        poly(p["armor_dark"], [(-14, -36), (14, -36), (16, -31),
             (15, -21), (12, -9), (10, 6), (-10, 6), (-12, -9),
             (-15, -21), (-16, -31)], False)
        # panel dada tengah (lebar, bersih) + highlight kiri-atas
        poly(p["armor_mid"], [(-11, -35), (11, -35), (13, -28),
             (11, -12), (8, 4), (-8, 4), (-11, -12), (-13, -28)], False)
        poly(p["armor_light"], [(-10, -34), (-3, -35), (0, -33),
             (-3, -16), (-8, -17), (-10, -26)], False)
        # medallion dada emas (garis tengah vertikal tipis)
        _NS_grimjaw._aaline(surface, p["gold_dark"], pt(-2, -33),
                            pt(-2, 2), 1)
        poly(p["gold_darkest"], [(-4, -18), (4, -18), (3, -10),
             (0, -8), (-3, -10)], False)
        poly(p["gold_mid"], [(-3, -17), (3, -17), (2, -11),
             (0, -9), (-2, -11)], False)
        # sash merah diagonal: bahu belakang-atas ke pinggang depan
        poly(p["red_darkest"], [(-17, -36), (-12, -39), (11, 5),
             (6, 9), (-13, -28)], False)
        poly(p["red_dark"], [(-15, -34), (-12, -37), (9, 5),
             (6, 8)], False)
        poly(p["red_mid"], [(-12, -33), (-10, -35), (7, 5),
             (5, 7)], False)
        _NS_grimjaw._aaline(surface, p["red_light"], pt(-14, -35),
                            pt(8, 6), 1)
        _NS_grimjaw._aaline(surface, p["armor_high"], pt(-16, -33),
                            pt(15, -33), 1)
        _NS_grimjaw._aaline(surface, p["armor_darkest"], pt(-10, -2),
                            pt(10, -2), 1)

        # ==== 6. SABUK + gesper emas + studs ====
        poly(p["armor_darkest"], [(-13, 7), (13, 7), (13, 14),
             (-13, 14)])
        poly(p["armor_dark"], [(-12, 8), (12, 8), (12, 13),
             (-12, 13)], False)
        _NS_grimjaw._aaline(surface, p["armor_light"], pt(-10, 9),
                            pt(10, 9), 1)
        for bx in (-8, 8):
            _NS_grimjaw._aacircle(surface, p["gold_mid"], pt(bx, 10),
                                  1, False)
        # gesper persegi besar
        poly(p["gold_darkest"], [(-4, 8), (4, 8), (4, 13), (-4, 13)],
             False)
        poly(p["gold_mid"], [(-3, 9), (3, 9), (3, 12), (-3, 12)],
             False)
        _NS_grimjaw._aacircle(surface, p["gold_light"], pt(-1, 10),
                              1, False)

        # ==== 7. LOINCLOTH DEPAN + penutup pinggul ====
        cloth_sway = int(math.sin(phase * 1.1) * 2)
        hem_spine = [(-7, 13), (-6, 20), (-3, 26 + cloth_sway),
                     (0, 28 + cloth_sway), (3, 25 + cloth_sway),
                     (6, 20), (7, 13)]
        hem = _NS_grimjaw._tuft_points(hem_spine, depth=2.5,
                                       min_len=4, seed=9)
        poly(p["red_darkest"], [(-7, 13)] + hem)
        poly(p["red_dark"], [(-5, 15), (-4, 20), (-2, 25 + cloth_sway),
             (0, 27 + cloth_sway), (2, 24 + cloth_sway), (4, 19),
             (5, 15)], False)
        for side in (-1, 1):
            hx = side * 13
            poly(p["armor_darkest"], [(hx - 5, 10), (hx + 4, 10),
                 (hx + 2, 26), (hx - 1, 28), (hx - 6, 24)], False)
            poly(p["armor_mid"], [(hx - 4, 11), (hx + 3, 11),
                 (hx + 1, 24), (hx - 2, 25), (hx - 5, 22)], False)
            _NS_grimjaw._aacircle(surface, p["gold_dark"], pt(hx - 1, 26),
                                  1, False)
            _NS_grimjaw._aacircle(surface, p["gold_light"], pt(hx - 2, 25),
                                  1, False)

        # ==== 8. PAULDRON: pelat belakang kecil + pelat depan besar ====
        for side in (-1, 1):
            sx = side * 15
            poly(p["metal_darkest"], [(sx - 9, -46), (sx + 9, -46),
                 (sx + 11, -32), (sx, -27), (sx - 11, -32)], False)
            poly(p["metal_dark"], [(sx - 8, -45), (sx + 8, -45),
                 (sx + 9, -33), (sx, -29), (sx - 9, -33)], False)
        for side in (-1, 1):
            sx = side * 16
            poly(p["metal_darkest"], [(sx - 12, -48), (sx + 12, -48),
                 (sx + 13, -34), (sx, -27), (sx - 13, -34)], False)
            poly(p["metal_mid"], [(sx - 10, -46), (sx + 10, -46),
                 (sx + 11, -35), (sx, -29), (sx - 11, -35)], False)
            # facet atas (key light) + rim emas bawah
            poly(p["metal_light"], [(sx - 10, -47), (sx - 2, -47),
                 (sx, -43), (sx - 8, -42)], False)
            poly(p["gold_dark"], [(sx - 12, -34), (sx + 12, -34),
                 (sx + 3, -28), (sx - 3, -28)], False)
            _NS_grimjaw._aaline(surface, p["gold_mid"], pt(sx - 11, -34),
                                pt(sx + 11, -34), 1)
            for sp in (-7, 4):
                poly(p["metal_darkest"],
                     [(sx + sp - 2, -48), (sx + sp, -56),
                      (sx + sp + 2, -48)], False)
                poly(p["metal_light"],
                     [(sx + sp - 1, -48), (sx + sp, -54),
        ...
        # ==== 9. MASK putih + kepala (dua celah mata besar) ====
        _NS_grimjaw._draw_elite_mask(surface, pt, poly, dot, f, phase,
                                     action, ap, detail, ward=ward,
                                     crit=crit)

        # ==== 10. LENGAN DEPAN (lengan pedang, di depan badan) ====
        grip = _NS_grimjaw._blade_grip_local(action, ap, phase)
        front_shoulder = (13, -33)
        if attack:
            front_elbow = _NS_grimjaw._front_arm_elbow(ap)
            if ap >= _NS_grimjaw.ATTACK_SWING_END:
                # Recovery: helper berakhir di (12,11), pose jaga rig ini
                # bersiku (15,-13). Blend halus supaya frame akhir = idle.
                rec = max(0.0, min(1.0, t_rec))
                mix = rec * rec * (3.0 - 2.0 * rec)
                ex = front_elbow[0] + int((15 - front_elbow[0]) * mix)
                ey = front_elbow[1] + int((-13 + int(breath) -
                                           front_elbow[1]) * mix)
                front_elbow = (ex, ey)
        elif spin:
            front_elbow = (20, -20)
        elif walk:
            front_elbow = (15, -14 + int(stride * 2))
        else:
            front_elbow = (15, -13 + int(breath))
        limb(front_shoulder, front_elbow, 10, p["skin_dark"], p["skin_mid"])
        limb(front_elbow, grip, 8, p["skin_mid"], p["skin_high"])
        # bracer baja di siku
        exx, exy = pt(*front_elbow)
        _NS_grimjaw._rect(surface, p["metal_darkest"],
                          (int(exx) - 5, int(exy) - 5, 11, 10),
                          border_radius=2)
        _NS_grimjaw._rect(surface, p["metal_mid"],
                          (int(exx) - 4, int(exy) - 4, 9, 8),
                          border_radius=2)
        _NS_grimjaw._aaline(surface, p["gold_mid"],
                            (int(exx) - 4, int(exy) + 3),
                            (int(exx) + 5, int(exy) + 3), 1)
        # gauntlet + tangan menggenggam
        dot(p["armor_darkest"], *grip, 6)
        dot(p["armor_dark"], *grip, 5)
        dot(p["armor_mid"], *grip, 4)
        _NS_grimjaw._aacircle(surface, p["skin_high"],
                              pt(grip[0] - 1, grip[1] - f), 2, False)

        # ==== 11. FLAME BLADE + smear/spin (helper lama dipertahankan) ====
        ...
        if attack and 0.30 < ap < 0.88 and not _NS_grimjaw._FX_LIVE.v:
            _NS_grimjaw._draw_blade_swing_trail(surface, cx, cy, f, phase,
                                                ap)
        if spin:
            _NS_grimjaw._draw_spin_flame_sweep(surface, cx, cy, f,
                                               spin_phase, phase)
        _NS_grimjaw._draw_elite_flame_blade(surface, pt, f, grip, angle,
                                            length, phase, detail,
                                            hot=hot, omni=omni)

        # ==== 12. FRINGE API DEPAN (forelock + side flame) ====
        ...
        # ==== 13. RIM LIGHT tipis (hanya sisi cahaya kiri-atas) ====
        rim = 0.6 + 0.4 * math.sin(phase * 1.9)
        _NS_grimjaw._aaline(surface, (*p["mask_shine"], int(90 * rim)),
                            pt(-10, -66), pt(-6, -69), 1)
        _NS_grimjaw._aaline(surface, (*p["armor_high"], int(70 * rim)),
                            pt(-18, -38), pt(-20, -32), 1)
        _NS_grimjaw._aacircle(surface, (*p["fire_light"], int(90 * rim)),
                              pt(-20, -72), 1)
    ...
    # -------------------------------------------------------------------
    def _draw_grimjaw_masterwork_details(surface, pt, poly, f, phase,
                                         action):
        """Detail portrait-only: serat api mahkota, engraving emas, jahitan
        armor, retak mask halus + ember naik.  Arena LOD (detail=False)
        tidak memuat pass ini.
        """
        p = _NS_grimjaw.PALETTE
        # serat mahkota api
        for i in range(6):
            ...
            _NS_grimjaw._aaline(surface, p["hair_shine"], pt(bx, by),
                                pt(tx, ty), 1)
        # retak mask halus
        _NS_grimjaw._aaline(surface, p["mask_line"], pt(-8, -50),
                            pt(-4, -55), 1)
        _NS_grimjaw._aaline(surface, p["mask_line"], pt(-4, -55),
                            pt(0, -48), 1)
        # engraving pauldron
        for side in (-1, 1):
            _NS_grimjaw._aaline(surface, p["gold_engrave"],
                                pt(side * 13, -31), pt(side * 19, -33), 1)
            _NS_grimjaw._aaline(surface, p["gold_engrave"],
                                pt(side * 13, -27), pt(side * 18, -28), 1)
        # jahitan armor dada
        for i in range(3):
            _NS_grimjaw._aaline(surface, p["armor_high"],
                                pt(-10 + i * 7, -26), pt(-7 + i * 7, -22),
                                1)
        # detail sabuk
        _NS_grimjaw._aaline(surface, p["gold_engrave"], pt(0, 10),
                            pt(0, 12), 1)
        _NS_grimjaw._aacircle(surface, p["gold_light"], pt(0, 9), 1, False)
        # ember naik (potret lebih kaya)
        for i in range(4):
...
class _NS_kaizen:
    ...
    def _draw_kaizen_elite(surface, cx, cy, facing, phase, action,
                           ...
                           storm=False):
        """Kaizen v5 — clean, readable pixel-art samurai, rewritten from zero.

        Design rules:
          * one confident silhouette (no scribbly spikes / floating shards)
          * low internal noise: a few large material planes, no 1px confetti
          * pauldron behind the shoulder, katana clean, face ¾ profile
          * same public geometry helpers used by the live FX layer
        """
        p = _NS_kaizen.PALETTE
        f = 1 if facing >= 0 else -1
        walk = action == "walk"
        ...
        sway = int(math.sin(phase * .8) * 3.0) * f if not (walk or attack) else 0
        pose = None
        lean = 0
        if walk:
            ...
            lean = int(pose["lean"] * .8) * f
            root_y += int(pose["bob"] * .7)
        tremble = 0
        ...
        amp = 4 if (walk or attack) else 2
        hair_wave = int(math.sin(phase * 1.25 - .7) * amp)
        scarf_wave = int(math.sin(phase * 1.45 - .9) * (amp + 2))
        band_wave = int(math.sin(phase * 1.6 - 1.1) * amp)

        # ════════════════════════════════════════════════════════════════
        # 1. BACK LAYER  —  compact hair, scarf tails, saya, back plate
        # ════════════════════════════════════════════════════════════════
        # Hair: a compact swept mass behind the skull + two ponytail locks.
        hair = [(-4, -84 + hair_wave), (-12, -90 + hair_wave),
                (-22, -90 + hair_wave), (-29, -84 + hair_wave),
                (-31, -73 + hair_wave), (-27, -63 + hair_wave),
                (-18, -58 + hair_wave), (-10, -62 + hair_wave),
                (-6, -74 + hair_wave)]
        poly(p["hair_darkest"], hair)
        _NS_kaizen._poly(surface, p["hair_dark"],
                         [pt(*a) for a in
                          [(-6, -82 + hair_wave), (-14, -88 + hair_wave),
                           (-24, -88 + hair_wave), (-28, -82 + hair_wave),
                           (-29, -72 + hair_wave), (-25, -65 + hair_wave),
                           (-18, -62 + hair_wave), (-10, -68 + hair_wave)]])
        poly(p["hair_mid"],
             [(-9, -80 + hair_wave), (-16, -85 + hair_wave),
              (-23, -84 + hair_wave), (-25, -76 + hair_wave),
              (-20, -70 + hair_wave), (-13, -74 + hair_wave)], False)
        # two clean ponytail locks flowing down-behind.
        poly(p["hair_darkest"],
             [(-18, -58 + hair_wave), (-30, -48 + hair_wave),
              (-35, -36 + hair_wave), (-28, -46 + hair_wave),
              (-15, -52 + hair_wave)], False)
        poly(p["hair_darkest"],
             [(-14, -64 + hair_wave), (-26, -50 + hair_wave),
              (-27, -28 + hair_wave), (-22, -42 + hair_wave),
              (-10, -56 + hair_wave)], False)
        _NS_kaizen._aacircle(surface, p["hair_light"],
                             pt(-24, -82 + hair_wave), 1)
        _NS_kaizen._aacircle(surface, p["hair_shine"],
                             pt(-19, -78 + hair_wave), 1)

        # scarf ribbon (thin, flows behind the shoulder)
        poly(p["scarf_deep"],
             [(-8, -52), (-15, -46), (-24, -38 + scarf_wave),
              (-28, -24 + scarf_wave), (-22, -12 + scarf_wave),
              (-15, -22), (-9, -34)], False)
        _NS_kaizen._aaline(surface, p["scarf_high"], pt(-15, -46),
                           pt(-25, -26 + scarf_wave), 1)

        # saya (short, clean red scabbard)
        limb((-9, 8), (-38, 28), 8, p["saya_dark"], None)
        _NS_kaizen._aaline(surface, p["saya_mid"], pt(-11, 6), pt(-40, 26), 5)
        _NS_kaizen._aaline(surface, p["saya_light"], pt(-12, 5), pt(-39, 24), 2)
        _NS_kaizen._aaline(surface, p["saya_shine"], pt(-13, 5), pt(-41, 25), 1)
        _NS_kaizen._aacircle(surface, p["gold_dark"], pt(-40, 28), 3)
        _NS_kaizen._aacircle(surface, p["gold_mid"], pt(-41, 27), 2)

        # rear shoulder plate (behind everything, top-left)
        poly(p["steel_dark"], [(-22, -42), (-15, -51), (-5, -51),
                               (0, -44), (-6, -37), (-17, -36)])
        poly(p["steel_mid"], [(-18, -42), (-13, -48), (-5, -48),
                              (-2, -42), (-7, -38), (-15, -37)], False)
        _NS_kaizen._aaline(surface, p["steel_light"], pt(-17, -44),
                           pt(-8, -48), 1)
        _NS_kaizen._aacircle(surface, p["steel_shine"], pt(-11, -45), 1)

        # ════════════════════════════════════════════════════════════════
        # 2. TORSO  —  clean jacket, closed V collar, sash, shoulder cap
        # ════════════════════════════════════════════════════════════════
        poly(p["cloth_deep"], [(-20, -46), (-8, -54), (14, -51),
                               (20, -36), (18, -12), (8, 4),
                               (-16, 4), (-22, -18)])
        poly(p["cloth_dark"], [(-17, -42), (-7, -49), (12, -47),
                               (17, -34), (15, -14), (7, 4),
                               (-14, 3), (-18, -18)], False)
        poly(p["cloth_mid"], [(-15, -40), (-7, -45), (2, -45),
                              (3, -18), (-5, 1), (-13, 0),
                              (-15, -18)], False)
        # closed V collar (a small skin wedge at the neck only).
        poly(p["skin_mid"], [(0, -46), (8, -48), (6, -42),
                             (1, -41), (-2, -43)], False)
        poly(p["cloth_darkest"], [(0, -50), (8, -48), (8, -44),
                                  (0, -42), (-8, -44), (-8, -49)])
        _NS_kaizen._aaline(surface, p["cloth_high"], pt(-6, -48),
                           pt(6, -49), 1)
        _NS_kaizen._aaline(surface, p["cloth_light"], pt(-13, -24),
                           pt(-12, 2), 1)
        # diagonal strap across the chest
        _NS_kaizen._aaline(surface, p["leather_dark"], pt(-3, -40),
                           pt(16, -10), 2)
        _NS_kaizen._aaline(surface, p["leather_mid"], pt(-2, -39),
                           pt(15, -11), 1)
        # obi
        poly(p["saya_dark"], [(-15, 4), (23, 6), (25, 15), (-13, 15)])
        poly(p["wrap_dark"], [(-14, 6), (23, 8), (23, 13), (-14, 12)], False)
        _NS_kaizen._aaline(surface, p["wrap_light"], pt(-8, 8),
                           pt(17, 10), 1)
        # front shoulder cap on the sword side (kept small).
        poly(p["cloth_darkest"], [(12, -45), (20, -40), (18, -28),
                                  (11, -30), (8, -38)])
        _NS_kaizen._aaline(surface, p["cloth_high"], pt(13, -42),
                           pt(19, -37), 1)

        # ════════════════════════════════════════════════════════════════
        # 3. HEAD + NECK + COLLAR + FACES
        # ════════════════════════════════════════════════════════════════
        poly(p["skin_dark"], [(1, -58), (10, -58), (12, -50),
                              (6, -47), (-1, -50)])
        # scarf collar
        poly(p["scarf_deep"], [(-8, -52), (11, -52), (14, -45),
                               (0, -39), (-11, -43), (-12, -48)])
        poly(p["scarf_dark"], [(-6, -50), (9, -50), (11, -45),
                               (-1, -41), (-9, -43), (-10, -46)], False)
        _NS_kaizen._aaline(surface, p["scarf_light"], pt(-5, -50),
                           pt(8, -50), 1)
        _NS_kaizen._aaline(surface, p["scarf_high"], pt(-4, -51),
                           pt(7, -51), 1)

        # face: ¾ front view, compact, with two clear eyes.
        face = [(-9, -84), (-2, -87), (8, -86), (14, -82),
                (16, -74), (15, -66), (11, -59), (4, -56),
                (-4, -57), (-9, -63), (-11, -74)]
        poly(p["skin_mid"], face)
        poly(p["skin_dark"],
             [(9, -56), (14, -61), (16, -68), (14, -74), (11, -71),
              (10, -63)], False)
        _NS_kaizen._aaline(surface, p["skin_light"], pt(-7, -82),
                           pt(10, -80), 1)
        # nose / jaw shading for the ¾ read.
        _NS_kaizen._aaline(surface, p["skin_darkest"], pt(16, -71),
                           pt(17, -63), 1)
        _NS_kaizen._aaline(surface, p["ink"], pt(17, -62), pt(10, -60), 1)

        # eyes: two clean sockets under the brow.
        def eye(ax):
            exs, eys = pt(ax, -67)
            _NS_kaizen._aacircle(surface, p["eye_white"], (exs, eys), 3)
            _NS_kaizen._aacircle(surface, p["white"], (exs, eys - 1), 1)
            _NS_kaizen._aacircle(surface, p["eye_iris"], (exs - 1, eys), 2)
            _NS_kaizen._aacircle(surface, p["eye_pupil"], (exs - 1, eys), 1)
            _NS_kaizen._aacircle(surface, p["eye_iris_light"],
                                 (exs - 2, eys - 1), 1)
            _NS_kaizen._aaline(surface, p["ink"], (exs + 1, eys - 3),
                               (exs - 3, eys - 3), 1)
        eye(-2)
        eye(9)
        # hachimaki across the forehead; eyes stay visible below it.
        poly(p["scarf_deep"], [(-11, -78), (16, -76), (17, -71),
                               (4, -68), (-12, -70)])
        poly(p["scarf_dark"], [(-9, -76), (14, -74), (15, -71),
                               (4, -70), (-10, -71)], False)
        _NS_kaizen._aaline(surface, p["scarf_light"], pt(-8, -75),
                           pt(13, -73), 1)
        bead = pt(4, -74)
        _NS_kaizen._aacircle(surface, p["gold_dark"], bead, 3)
        _NS_kaizen._aacircle(surface, p["gold_mid"], bead, 2)
        ...
        for k, (drop, ln) in enumerate(((-2, 16), (3, 12))):
            ...
            poly(p["scarf_dark"],
                 [(-11, -77 + k * 2), (-17, -76 + wv // 2),
                  ...
                  (-25, -64 + drop + wv), (-17, -70)], False)
            _NS_kaizen._aaline(surface, p["scarf_mid"], pt(-17, -74 + wv // 2),
                               pt(-30, -67 + drop + wv), 1)

        # topknot: small and clean (preferred "clean-scalp" read).
        poly(p["hair_darkest"], [(-1, -87), (2, -91), (6, -88),
                                 (4, -84), (0, -85)])
        _NS_kaizen._aacircle(surface, p["hair_light"], pt(1, -89), 1)
        ...
        poly(p["pants_dark"],
             [(dx, dy) for dx, dy in
              _NS_kaizen._tuft_points(back_hem, 2.4, 4.5, seed=21)])
        front_hem = [(-2, 16), (23, 12), (32, 25), (25, 39), (4, 42),
                     (-2, 34)]
        poly(p["pants_mid"],
             [(dx, dy) for dx, dy in
              _NS_kaizen._tuft_points(front_hem, 2.4, 4.5, seed=33)])
        poly(p["pants_light"], [(2, 18), (20, 15), (27, 27), (20, 36),
                                (6, 36)], False)
        for dx in (-14, 2, 8, 17):
            _NS_kaizen._aaline(surface, p["cloth_darkest"], pt(dx, 14),
                               pt(dx + (2 if dx > 0 else -2), 32), 1)
        _NS_kaizen._dither_dots(surface, p["pants_light"], pt(7, 24),
                                pt(19, 21), step=3, alpha=150)

        for hip, knee, foot, shade, lift in (
                ((-11, 20), (-11, 36), rear_foot, p["pants_dark"], rear_lift),
                ((11, 20), (11, 35), front_foot, (52, 70, 130), front_lift)):
            knee = (knee[0], knee[1] - lift)
            poly(shade, [hip, (hip[0] + 8, hip[1]),
                         (knee[0] + 7, knee[1]), (foot[0] + 5, foot[1] - 8),
                         (foot[0] - 5, foot[1] - 8),
                         (knee[0] - 6, knee[1])])
            poly((204, 211, 216), [(foot[0] - 5, foot[1] - 14),
                 (foot[0] + 5, foot[1] - 14), (foot[0] + 6, foot[1] - 5),
                 (foot[0] - 6, foot[1] - 5)], False)
            _NS_kaizen._aaline(surface, (160, 170, 178),
                               pt(foot[0] - 4, foot[1] - 10),
                               pt(foot[0] + 5, foot[1] - 10), 1)
            poly(p["leather_dark"], [(foot[0] - 7, foot[1] - 5),
                 (foot[0] + 9, foot[1] - 5), (foot[0] + 11, foot[1]),
                 (foot[0] - 7, foot[1])])
            _NS_kaizen._aaline(surface, p["leather_mid"],
                               pt(foot[0] - 5, foot[1] - 3),
                               pt(foot[0] + 7, foot[1] - 3), 1)
            if foot is front_foot:
                _NS_kaizen._aaline(surface, (235, 242, 248),
                                   pt(foot[0] - 4, foot[1] - 9),
                                   pt(foot[0] - 4, foot[1] - 3), 1)
        ...
        limb((14, -40), (24, -22), 9, p["cloth_dark"], p["cloth_light"])
        limb((24, -22), hand, 7, p["skin_dark"], p["skin_light"])
        ...
        smear = 0.0
        if attack:
            apc = max(0.0, min(1.0, attack_progress))
            glow = max(glow, .55 + .45 * math.sin(apc * math.pi))
            smear = max(0.0, 1.0 - abs(apc - .5) / .22)
        if gale:
            glow = max(glow, .5)
        if storm:
            glow = max(glow, .65)
        ...
        if walk:
            ...
            if contact > 0:
                ...
                for i in range(3):
                    _NS_kaizen._aacircle(
                        surface, (*p["wind_mid"], max(15, alpha - i * 30)),
                        (fx_ - f * (4 + i * 4), fy_ - i % 2),
                        max(1, 3 - i // 2))
            for i in range(2):
                sy_ = cy - 8 + i * 16 + root_y
                _NS_kaizen._aaline(surface, (*p["wind_light"], 80 - i * 22),
                                   (cx - f * (40 + i * 8), sy_),
                                   (cx - f * (20 + i * 6), sy_ - 1), 1)
        elif attack:
            apc = max(0.0, min(1.0, attack_progress))
            impact = max(0.0, 1.0 - abs(apc - .54) / .16)
            if impact > 0.08:
                tip = _NS_kaizen._katana_tip_local(phase, "attack", apc)
                ix, iy = pt(tip[0], tip[1])
                _NS_kaizen._spark_star(surface, ix, iy,
                                       int(10 + 16 * impact), p["wind_pale"],
                                       int(230 * impact), 6, rot=.4,
                                       core=p["white"])
                _NS_kaizen._ellipse(
                    surface, (*p["wind_bright"], int(140 * impact)),
        ...
        # rim light: only a few clean lines on the lit edge.
        pulse2 = 0.72 + 0.28 * math.sin(phase * 1.8)
        rim_a = int(130 * pulse2)
        rim_col = (*p["wind_light"], rim_a)
        rim_hot = (*p["wind_white"], int(180 * pulse2))
        _NS_kaizen._aaline(surface, rim_col, pt(12, -80), pt(15, -70), 1)
        _NS_kaizen._aaline(surface, rim_hot, pt(13, -79), pt(14, -73), 1)
        _NS_kaizen._aaline(surface, rim_col, pt(15, -38), pt(20, -20), 1)
        _NS_kaizen._aaline(surface, rim_col, pt(15, 35 - front_lift),
                           pt(16, 54 - front_lift), 1)
        _NS_kaizen._aacircle(surface, rim_hot, pt(16, 57 - front_lift), 1)

        if detail:
            # portrait-only micro-detail (kept out of the gameplay cache LOD)
            for i in range(3):
                _NS_kaizen._aaline(
                    surface, p["hair_light"],
                    pt(-10 - i * 3, -82 + i * 2),
                    pt(-20 - i * 3, -84 + i * 4), 1)
            _NS_kaizen._aacircle(surface, p["skin_high"], pt(1, -61), 1)
            _NS_kaizen._aacircle(surface, p["skin_light"], pt(5, -57), 1)
            _NS_kaizen._aacircle(surface, p["wind_bright"], pt(14, -66), 1)
            _NS_kaizen._aaline(surface, p["cloth_light"], pt(-13, -28),
                               pt(-12, -4), 1)
            _NS_kaizen._aacircle(surface, p["white"], bead, 1)
            for a in (-.8, -.3, .2):
                _NS_kaizen._aaline(surface, p["steel_mid"], pt(-14, -48),
                                   pt(-14 + math.cos(a) * 6,
                                      -48 + math.sin(a) * 6), 1)
            _NS_kaizen._aaline(surface, p["saya_shine"], pt(-17, 7),
                               pt(-30, 19), 1)
    ...
    def _draw_swordsman_rim_light(surface, x, y, phase):
        ...
        def build():
            halo = pygame.Surface((108, 120), pygame.SRCALPHA)
            for radius, alpha in ((40, 4), (30, 7), (20, 10)):
                _NS_kaizen._aacircle(halo, (*_NS_kaizen.PALETTE["wind_dark"],
                                             int(alpha)), (54, 60), radius)
            _NS_kaizen._aaline(halo, (*_NS_kaizen.PALETTE["wind_mid"], 26),
                               (36, 72), (18, 81), 2)
            _NS_kaizen._aaline(halo, (*_NS_kaizen.PALETTE["wind_light"], 24),
                               (70, 70), (96, 58), 1)
            return halo
        halo = _NS_kaizen._static("rim_halo", build)
        old_a = halo.get_alpha()
        halo.set_alpha(int(255 * pulse))
        surface.blit(halo, (x - 54, y - 60))
        halo.set_alpha(old_a if old_a is not None else 255)

    def _draw_wind_aura(surface, x, y, phase):
        def build():
            aura = pygame.Surface((220, 200), pygame.SRCALPHA)
            for radius in range(80, 6, -4):
                alpha = int((80 - radius) * 0.8)
                if alpha > 0:
