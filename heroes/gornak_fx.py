...
class ParticleSystem:
    ...
    def stream(self, x, y, tx, ty, count, life=(0.3, 0.6), size=(1, 3),
        ...
        for i in range(min(int(count), 24)):
            ...
            spd = d / random.uniform(life[0], life[1])
...
class SwingTrail:
    ...
    # ------------------------------------------------------------------
    #: satu langkah trail tidak boleh membelok lebih tajam dari ini tanpa
    #: memutus strip (arah balik recovery = strip baru, bukan sapuan)
    MAX_TURN_DOT = 0.20

    def _ribbon(self, surface, lst, edge, core, tip_col, base_alpha):
        """Satu pita: jalur UJUNG bilah, melebar di ujung baru -> nol di ekor.

        Bukan kipas dari pangkal senjata. Kipas terlihat seperti "lembaran"
        raksasa begitu ayunan berbalik (bilah recovery menyapu badan), dan
        itulah yang membuat slash trail murah terbaca. Di sini yang
        dihubungkan hanya lintasan ujung bilah, dan lintasan itu DIPUTUS di
        tiap titik di mana arahnya berbalik — jadi trail selalu terbaca
        sebagai bekas tebasan, tidak pernah sebagai bayangan polygon.

        Lebar pita diukur SEPARUH sejajar bilah (ke luar dari ujung, ke
        dalam ke arah grip) supaya bentuknya ikut orientasi senjata, bukan
        garis tebal tegak lurus yang lepas dari bilah.
        """
        n = len(lst)
        if n < 2:
            return
        # ── pecah histori jadi strip-strip yang berkesinambungan ─────
        strips = []
        cur = []
        prev = None
        for i in range(n):
            ...
            if cur:
                ...
                ln = d.length()
                if ln < 0.75:
                    continue                      # bilah nyaris diam
                d *= 1.0 / ln
                if prev is not None and (d.x * prev.x + d.y * prev.y) \
                        < self.MAX_TURN_DOT:
                    strips.append(cur)             # arah berbalik -> putus
                    cur = []
                prev = d
            cur.append((i, pygame.Vector2(g), pygame.Vector2(t), age))
        if cur:
            strips.append(cur)
        ...
        for si in range(len(strips)):
            strip = strips[si]
            # Buang sampel paling tua sampai total sapuan masuk MAX_SWEEP.
            # Sampel yang lebih tua berarti bilah sudah lewat di jalur yang
            # sama -> kalau tetap digambar, trail melingkari badan dan
            # berubah jadi "tembok" transparan (dilarang master prompt).
            total = 0.0
            keep = 0
            for j in range(len(strip) - 1, 0, -1):
                total += abs(strip[j][4])
            ...
            if m < 2:
                continue
            xs = [q[2].x for q in strip] + [q[1].x for q in strip]
            ys = [q[2].y for q in strip] + [q[1].y for q in strip]
            ...
            edges_out, edges_in = [], []
            for j, (_i, g, t, age) in enumerate(strip):
                fade = max(0.0, 1.0 - age / self.life)
                rank = (j + 1) / float(m)             # 0 = ekor, 1 = baru
                k = (rank ** 2) * fade
                if k <= 0.004:
                    edges_out.append(None)
                    edges_in.append(None)
                    continue
                blade = t - g
                bl = blade.length()
                if bl < 1.0:
                    ...
                    edges_in.append(None)
                    continue
                u = blade * (1.0 / bl)                # arah pangkal -> ujung
                wide = (0.10 + 0.46 * rank) * min(64.0, bl) * \
                    self.width_boost
                outer = t + u * (0.16 * wide + 1.5)
                inner = t - u * wide
                edges_out.append((outer, inner, k, fade))
            for j in range(m - 1):
                a, b = edges_out[j], edges_out[j + 1]
                if a is None or b is None:
                    continue
                ao, ai, ka, _fa = a
                bo, bi, kb, _fb = b
                k = max(ka, kb)
                a_edge = int(base_alpha * k)
                if a_edge > 5:
                    pygame.draw.polygon(buf, (*edge, a_edge),
                                        [loc(ao), loc(ai), loc(bi), loc(bo)])
                a_core = int((base_alpha + 62) * k)
                if a_core > 8:
                    mo = ao.lerp(ai, 0.30)
                    mi = ai.lerp(ao, 0.16)
                    no = bo.lerp(bi, 0.30)
                    nb = bi.lerp(bo, 0.16)
                    pygame.draw.polygon(buf, (*core, min(255, a_core)),
                                        [loc(mi), loc(mo), loc(no), loc(nb)])
            # garis ujung: 1-2 px terang di sepanjang lintasan tip
            pts = [loc(strip[j][2]) for j in range(m)
                   if edges_out[j] is not None]
            if len(pts) >= 2:
                for j in range(1, len(pts)):
                    kk = (j / float(len(pts) - 1)) ** 2
                    pygame.draw.line(buf, (*tip_col, int(215 * kk)),
                                     pts[j - 1], pts[j], 2 if kk > 0.55 else 1)
            surface.blit(buf, (minx, miny))
...
class SwingTrail:
    ...
    # ------------------------------------------------------------------
    def _ribbon(self, surface, lst, edge, core, tip_col, base_alpha):
        """Gambar satu pita: badan gelap -> inti terang -> garis ujung 1 px."""
        n = len(lst)
        if n < 2:
            return
        xs, ys = [], []
        ...
        for i in range(n - 1):
            ...
            g1, t1, _a1 = lst[i + 1]
            fade = max(0.0, 1.0 - age / self.life)
            k = ((i + 1) / float(n)) ** 2 * fade
            a_edge = int(base_alpha * k * self.width_boost)
            if a_edge > 4:
                # badan pita: dari 25% pangkal bilah (bukan dari tangan),
                # supaya trail = jalur UJUNG bilah, bukan kipas lebar
                p0 = g0.lerp(t0, 0.25)
                p1 = g1.lerp(t1, 0.25)
                pygame.draw.polygon(buf, (*edge, a_edge),
                                    [loc(p0), loc(t0), loc(t1), loc(p1)])
            a_core = int((base_alpha + 55) * k)
            if a_core > 6:
                m0 = g0.lerp(t0, 0.62)
                m1 = g1.lerp(t1, 0.62)
                pygame.draw.polygon(buf, (*core, min(255, a_core)),
            ...
            a_tip = int(235 * k)
            if a_tip > 8:
                pygame.draw.line(buf, (*tip_col, a_tip), loc(t0), loc(t1), 2)
        surface.blit(buf, (minx, miny))
