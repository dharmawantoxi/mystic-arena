# ============================================================================
# heroes/gravewake_fx.py
# ----------------------------------------------------------------------------
# Lapisan FX HIDUP untuk Gravewake "The Tidehunter" (MINI BOSS level 6).
#
# Modul ini memegang SEMUA efek yang tidak boleh ikut ter-cache / ter-skala:
#   * partikel air (percikan, droplet, gelembung, busa, karat jangkar)
#   * pita tebasan jangkar (SwingTrail, dari histori posisi sungguhan)
#   * proyektil "Anchor Wave" (bulan sabit air + jangkar air) untuk Q
...
#
# Arsitektur mengikuti heroes/thalgryn_fx.py, heroes/kunkka_fx.py, dan
# heroes/syrentha_fx.py (rumah gaya V3 yang sama):
...
#   * semua geometry/pose dibaca dari bosses/level6._NS_gravewake supaya
...
#: Master switch lapisan FX Gravewake.
GRAVEWAKE_FX_ENABLED = True
...
#: Jarak (piksel dunia) di bawah ini serangan dasar Gravewake dibaca sebagai
#: tebasan jangkar yang menyambung ke target. = range di boss_data.py.
MELEE_REACH = 65

#: Durasi status skill (frame) - HARUS sinkron dengan bosses/base_boss.py
#: (_gravewake_q/w/e/r) dan hero_skills/_bundle.py.
...
# ============================================================================
# 1.  PALETTE  (nilai cadangan; disinkronkan dari _NS_gravewake.PALETTE)
# ============================================================================
...
_PALETTE_SYNC = {
    "skin_darkest": "skin_darkest",
    "skin_dark": "skin_dark",
    "skin_mid": "skin_mid",
    "skin_light": "skin_light",
    "skin_high": "skin_high",
    "skin_shine": "skin_shine",
    "belly_dark": "belly_dark",
    "belly_mid": "belly_mid",
    "belly_light": "belly_light",
    "bone_darkest": "bone_darkest",
    "bone_dark": "bone_dark",
    "bone_mid": "bone_mid",
    "bone_light": "bone_light",
    "bone_shine": "bone_shine",
    "leather_darkest": "leather_darkest",
    "leather_dark": "leather_dark",
    "leather_mid": "leather_mid",
    "leather_light": "leather_light",
    "anchor_darkest": "anchor_darkest",
    "anchor_dark": "anchor_dark",
    "anchor_mid": "anchor_mid",
    "anchor_light": "anchor_light",
    "anchor_shine": "anchor_shine",
    "anchor_rust": "anchor_rust",
    ...
    "teeth_dark": "teeth_dark",
    "teeth_mid": "teeth_mid",
    "teeth_light": "teeth_light",
    "eye_dark": "eye_dark",
    "eye_mid": "eye_mid",
...
def _sync_palette():
    """Salin warna tema dari ``_NS_gravewake.PALETTE`` sekali saja."""
...
def _renderer():
    """``_NS_gravewake`` atau None. Diimpor malas (modul boss besar)."""
    global _RENDERER
    if _RENDERER is None:
        try:
            from bosses.level6 import _NS_gravewake as G
            _RENDERER = G
...
def _fallback_pose(boss):
    ...
    action = getattr(boss, "_gw_pose_action", None)
    if action is None:
        action = {"q": "smash", "w": "tide", "e": "shell",
                  "r": "ravage"}.get(skill)
    if action is None:
        action = ("attack" if getattr(boss, "_gw_attack_active", False)
                  else "idle")
    phase = float(getattr(boss, "pulse", 0.0) or 0.0)
    ap = 0.0
    if action == "attack":
        raw = float(getattr(boss, "_gw_attack_progress", 0.0) or 0.0)
...
def pose_of(boss):
    ...
    try:
        return G._resolve_pose(boss, bool(getattr(boss, "_gw_moving_cached",
                                                  False)))
    except Exception:                          # pragma: no cover
        return _fallback_pose(boss)
...
def screen_point(boss, x, y, local, action=None, phase=None, ap=None):
    """Ruang lokal rig -> piksel layar (sama dengan _NS_gravewake._local)."""
    G = _renderer()
...
def anchor_points(boss, x, y):
    """(grip, tip) jangkar dalam piksel layar — sumber bentuk trail."""
    G = _renderer()
    action, phase, ap = pose_of(boss)
    if G is None:
        grip_l = (17, 6)
...
# ============================================================================
# 6.  SWING TRAIL  —  jejak jangkar dari histori posisi sungguhan
# ============================================================================

class SwingTrail:
    """Pita tebasan jangkar berbasis histori (grip, tip) per frame."""
    ...
    def __init__(self, samples=TRAIL_SAMPLES):
        ...
        self.active = False
        self.color_edge = P["water_mid"]
        self.color_core = P["water_light"]
...
class ImpactFX:
    ...
    ``kind`` memilih bahasa visual:
        ``anchor``      — tebasan jangkar (baja + percikan air)
        ``wave``        — Anchor Wave (semburan horizontal air)
        ``tide``        — Tidebringer (cincin + tiang air)
        ``shell``       — Kraken Shell (kabut biru + cincin ganda)
        ``ravage``      — Ravage (spiral + bintang stun)
    """
    ...
    def __init__(self, x, y, angle=0.0, power=1.0, crit=False,
                 kind="anchor", ground=0.0, seed=None):
        self.x = float(x)
        self.y = float(y)
        ...
        self.crit = bool(crit)
        self.active = True
        self.kind = kind if kind in self.KINDS else "anchor"
    ...
    def _ramp(self):
        if self.kind == "anchor":
            return P["water_dark"], P["water_light"], P["anchor_shine"]
        if self.kind == "wave":
            return P["water_dark"], P["water_mid"], P["water_bright"]
        if self.kind == "tide":
            return P["water_dark"], P["water_light"], P["water_hot"]
        if self.kind == "shell":
            return P["water_darkest"], P["water_light"], P["water_white"]
        if self.kind == "ravage":
            return P["water_dark"], P["water_light"], P["star_yellow"]
    ...
    def draw(self, surface):
        ...
        a = int(210 * inv * inv)
        if a > 6 and rr > 4:
            if self.kind in ("anchor", "wave"):
                er = ellipse_ring_surface(rr, max(3, int(rr * 0.55)), th,
                                          mid, deg)
            elif self.kind in ("tide", "ravage"):
            ...
            if rr2 > 4:
                er2 = ring_surface(rr2, max(1, th - 1), hot, 255,
                                   dashed=8 if self.kind not in ("anchor",
                                                                 "wave")
                                   else 0)
        ...
        # 4. SLASH FRAGMENT (anchor / wave)
        if t < 0.52 and self.kind in ("anchor", "wave"):
            st = 1.0 - t / 0.52
            span = 0.5 + 0.5 * pw
        ...
        # 5. RETAKAN ZIGZAG (tide / ravage / shell)
        if t < 0.62 and self.kind in ("tide", "ravage", "shell"):
            st = 1.0 - t / 0.62
            n = 5 if self.kind != "ravage" else 7
            for i in range(n):
        ...
        # 6. TETES AIR (shell / ravage)
        if self.kind in ("shell", "ravage") and t < 0.7:
            st = 1.0 - t / 0.7
            for i in range(6):
...
# ============================================================================
# 8.  PROJECTILE SYSTEM  —  ANCHOR WAVE
# ============================================================================

class GravewakeProjectile:
    """Anchor Wave (Q) — sabit air + jangkar air yang melaju ke depan.
    ...
    def draw(self, surface):
        ...
        for i, pv in enumerate(self.trail):
            ...
            _shard_poly(surface, pv.x, pv.y, self.rotation, 14, 3,
                        P["water_dark"], a)
        draw_anchor_wave(surface, self.position.x, self.position.y,
...
class ProjectileSystem:
    ...
    def spawn(self, x, y, tx, ty, **kw):
        ...
        pr = GravewakeProjectile(x, y, tx, ty, **kw)
...
class SkillFX:
    """Efek skill Gravewake dengan lifecycle bertahap.
    ...
    TIMELINE = {
        ...
        "w": (0.25, 0.12, 0.25, 0.14, 0.18),
        "e": (0.20, 0.10, 0.24, 0.12, 0.16),
        "r": (0.35, 0.16, 0.30, 0.15, 0.22),
    }

    TINT = {
        "q": ((10, 70, 70), (170, 245, 235)),      # Anchor Smash: arus air
        "w": ((14, 62, 96), (180, 240, 250)),      # Tidebringer: tiang air
        "e": ((18, 80, 96), (200, 240, 250)),      # Kraken Shell: biru pucat
        "r": ((10, 52, 96), (255, 230, 90)),       # Ravage: putih + kuning
    }
        ...
        if self.phase == "charge":
            ...
            if kind == "e":
                ps.burst(self.x, self.y + self.ground, 4,
                         ...
                         colors=(P["water_light"], P["water_mid"], tint_d),
            ...
            if kind == "q":
                ...
                ps.stream(self.x, self.y,
                          ...
                          colors=(P["water_light"], P["water_mid"],
                                  P["water_darkest"]), bow=20.0, layer="back")
            elif kind == "w":
                ps.burst(self.x, self.y, 8, speed=(60, 160),
                         life=(0.2, 0.5), size=(2, 4),
                         colors=(P["water_bright"], P["water_light"],
                                 P["water_mid"]),
        ...
        if self.phase == "impact" and not self._impacted:
            ...
            if kind == "r":
                ps.burst(self.x, self.y - R * 0.3, int(14 + R / 16),
                         ...
                         colors=(P["star_yellow"], P["water_bright"],
    ...
    def draw_ground(self, surface):
        ...
        if self.phase in ("release", "area", "impact"):
            ...
            alpha = max(0, int(200 * (1.0 - spread * 0.5)))
            if rr > 8 and alpha > 10:
                ring = ring_surface(rr, max(1, int(4 * (1 - spread)) + 1),
                                    dark, alpha,
                                    dashed=10 if self.kind not in ("q", "w")
                                    else 0)
                surface.blit(ring, (x - ring.get_width() // 2,
    ...
    def draw_front(self, surface):
        ...
        # ── Q: semburan arus horizontal + jangkar air ───────────────
        if self.kind == "q" and self.phase in ("release", "area"):
            ...
            hx = x + math.cos(ang) * L
            hy = y + math.sin(ang) * L
            _draw_mini_anchor(surface, hx, hy, ang, int(200))
            pygame.draw.circle(surface, (*hot, 230), (int(hx), int(hy)), 4)
            pygame.draw.circle(surface, P["fx_white"], (int(hx), int(hy)), 2)

        # ── W: cincin konsentris + tiang air ────────────────────────
        if self.kind == "w" and self.phase in ("area", "impact"):
            k = max(0.0, min(1.0, (a - self.t_area)
                              / max(0.05, self.t_impact - self.t_area)))
            rr = int(30 + 30 * k)
            ring = ring_surface(rr, 3, P["water_light"],
                                int(200 * (1 - k)), 0)
            surface.blit(ring, (x - ring.get_width() // 2,
                                y - ring.get_height() // 2))
            for i in range(6):
                base_angle = a * 1.5 + i * math.pi / 3
                tx = x + int(math.cos(base_angle) * (rr + 8))
                ty = y - int((a * 30) % 40) \
                    + int(math.sin(base_angle) * 8)
                _draw_pillar_fx(surface, tx, ty, a + i, int(210 * (1 - k)))

        # ── E: gelembung + cangkang ─────────────────────────────────
        if self.kind == "e" and self.phase in ("release", "area"):
            k = max(0.0, min(1.0, (a - self.t_release)
                              / max(0.05, self.t_area - self.t_release)))
            for i, (ox, oy) in enumerate(((-60, 14), (60, 14), (-36, -22))):
                delay = i * 0.1
                if k < delay:
                    continue
                ip = min(1.0, (k - delay) / max(0.1, 1 - delay))
                mx = x + ox + int(math.sin(a + i) * 3)
                my = y + oy + int(math.sin(a * 1.2 + i) * 3)
                alpha = int(200 * min(1.0, ip * 2))
                _draw_bubble_fx(surface, mx, my, a + i, alpha)

        # ── R: spiral + bintang stun + tentakel air ─────────────────
        if self.kind == "r" and self.phase in ("release", "area", "impact"):
            ...
            for i in range(10):
                ang2 = a * 1.2 + i * 0.5
                r = 20 + i * 8
                nx = x + int(math.cos(ang2) * r)
                ny = y - 5 + int(math.sin(ang2) * r * 0.5)
                _draw_pillar_fx(surface, nx, ny, a + i,
                                int(220 * (1 - i / 10 * 0.3)))
            for i in range(7):
                ang3 = a * 0.8 + i * math.pi / 3.5
                r = 60 + int(math.sin(a * 2 + i) * 8)
                sx = x + int(math.cos(ang3) * r)
                sy = y - 18 + int(math.sin(ang3) * r * 0.5)
                _draw_star_fx(surface, sx, sy, 4, P["star_yellow"])


def _draw_mini_anchor(surface, cx, cy, angle, alpha=200):
    """Jangkar air mini di kepala Anchor Wave."""
    alpha = max(0, min(255, int(alpha)))
    _draw_anchor = None
    ax = int(cx)
    ay = int(cy)
    pygame.draw.line(surface, (*P["water_dark"], alpha), (ax, ay - 9),
                     (ax, ay + 7), 3)
    pygame.draw.line(surface, (*P["water_mid"], alpha), (ax, ay - 9),
                     (ax, ay + 7), 2)
    pygame.draw.line(surface, (*P["water_light"], alpha), (ax - 6, ay + 6),
                     (ax + 6, ay + 6), 2)
    pygame.draw.circle(surface, (*P["water_hot"], alpha), (ax, ay - 9), 2)
    for side in (-1, 1):
        pygame.draw.line(surface, (*P["water_mid"], alpha),
                         (ax + side * 6, ay + 6), (ax + side * 8, ay + 2), 2)


def _draw_pillar_fx(surface, cx, cy, phase, alpha=220):
    """Tiang air kecil (tidebringer / ravage)."""
    alpha = max(0, min(255, int(alpha)))
    h = 10
    for i in range(h):
        t = i / float(h)
        w = max(1, int(3 * (1 - t)))
        pygame.draw.rect(surface, (*P["water_dark"], alpha),
                         (cx - w, cy - i, w * 2, 2))
        pygame.draw.rect(surface, (*P["water_mid"], alpha),
                         (cx - w + 1, cy - i, w * 2 - 2, 1))
    pygame.draw.circle(surface, (*P["water_light"], alpha), (cx, cy - h), 2)
    pygame.draw.circle(surface, (*P["water_hot"], alpha), (cx, cy - h), 1)


def _draw_bubble_fx(surface, cx, cy, phase, alpha=200):
    """Gelembung air (kraken shell)."""
    alpha = max(0, min(255, int(alpha)))
    r = 6
    pygame.draw.circle(surface, (*P["water_dark"], alpha // 2),
                       (cx, cy), r + 2, 2)
    pygame.draw.circle(surface, (*P["water_light"], alpha),
                       (cx, cy), r, 1)
    pygame.draw.circle(surface, (*P["water_hot"], alpha),
                       (cx - 2, cy - 2), 1)
    pygame.draw.circle(surface, (*P["water_white"], alpha),
                       (cx + 2, cy + 2), 1)


def _draw_star_fx(surface, cx, cy, size=4, color=None):
    if color is None:
        color = P["star_yellow"]
    pygame.draw.line(surface, color, (cx, cy - size), (cx, cy + size), 2)
    pygame.draw.line(surface, color, (cx - size, cy), (cx + size, cy), 2)
    pygame.draw.line(surface, color, (cx - size // 2, cy - size // 2),
                     (cx + size // 2, cy + size // 2), 1)
    pygame.draw.line(surface, color, (cx - size // 2, cy + size // 2),
                     (cx + size // 2, cy - size // 2), 1)
    pygame.draw.circle(surface, P["white"], (cx, cy), 1)


# ============================================================================
# 10. DIRECTOR — mengikat semua sistem FX untuk satu unit Gravewake
# ============================================================================
...
class GravewakeFXDirector:
    """Mengikat particle + trail + proyektil + impact + skill FX untuk
    satu unit Gravewake (mini boss; kodenama sama untuk jalur hero)."""
    ...
    def on_swing_impact_frame(self, x, y, facing):
        """Tebasan jangkar di frame IMPACT (connect melee ke target)."""
        _g, tip = anchor_points(self.hero, x, y)
        ...
        if tgt is not None and getattr(tgt, "alive", False):
            ...
            if d <= MELEE_REACH:
                ...
                melee = True
        if melee:
            self.on_impact(tx, ty, ang, 1.0, False, kind="anchor")
        else:
            self.impacts.append(ImpactFX(tip.x, tip.y, ang, 0.5, False,
                                         kind="anchor", ground=0.0,
                                         seed=int(self.frames)))
        self.particles.burst(
            tip.x, tip.y, 7,
            speed=(110, 250), life=(0.1, 0.24), size=(1, 3),
            colors=(P["water_light"], P["fx_white"], P["water_bright"]),
    ...
    def on_impact(self, x, y, angle=0.0, power=1.0, crit=False,
        ...
        spark, edge, hot = (P["water_bright"], P["water_light"],
                            P["fx_white"])
        if kind == "anchor":
            spark, edge, hot = (P["water_light"], P["water_mid"],
                                P["anchor_shine"])
    ...
    def _resolve_state(self):
        ...
        if getattr(h, "_gw_attack_active", False):
            ph = attack_phase(float(getattr(h, "_gw_attack_progress", 0.0)))
            if ph in ("ANTICIPATION", "WINDUP"):
                return "CHARGE"
            if ph in ("SWING", "IMPACT"):
                return "SWING"
            return "ATTACK"
        if getattr(h, "_gw_moving_cached", False):
            return "RUN" if float(getattr(h, "speed", 1.0)) >= 2.2 \
                else "WALK"
        st = getattr(h, "_gw_state", None)
    ...
    def _update_state(self, dt):
        ...
        if want != self.state:
            ...
            if self.state != "DEATH" and (new_p >= cur_p
                ...
                self.state_time = 0.0
        self.state_time += dt
        self.anim_phase = str(getattr(self.hero, "_gw_attack_phase", None)
                              or "NONE")
    ...
    def update(self, dt, x, y):
        ...
        attacking = bool(getattr(h, "_gw_attack_active", False))
...
def should_freeze_frame():
    if not GRAVEWAKE_FX_ENABLED or _feel is None:
        return False
...
# ============================================================================
# 12.  ANCHOR WAVE RENDERER BERSAMA
# ============================================================================

def draw_anchor_wave(surface, px, py, angle=0.0, age=0, radius=26.0,
                     seed=0):
    """Bulan sabit air + jangkar air (Anchor Wave). Digambar langsung
    (tanpa transform.rotate)."""
    px, py = int(px), int(py)
    ...
    # Jangkar air mini di kepala gelombang
    tipx = px + cos_a * r
    tipy = py + sin_a * r
    _draw_mini_anchor(surface, tipx, tipy, angle, 210)
    pygame.draw.circle(surface, P["water_hot"], (tipx, tipy), 2)
...
def draw_debug_overlay(surface, director):
    ...
    try:
        ...
        x = int(getattr(h, "x", 0))
        y = int(getattr(h, "y", 0))
        r = max(6, int(getattr(h, "radius", 40) * 0.9))
        pygame.draw.rect(surface, (80, 170, 255, 150),
                         pygame.Rect(x - r, y - r - 8, r * 2, r * 2), 1)
        rng = max(10, int(getattr(h, "range", 65) * 0.9))
...
def director_for(hero):
    """Ambil (atau buat) director FX untuk satu unit Gravewake."""
    _sync_palette()
    d = getattr(hero, "_gw_fx", None)
    if d is None:
        d = GravewakeFXDirector(hero)
        try:
            hero._gw_fx = d
        except Exception:                      # pragma: no cover
            return d
...
def _release(director):
    try:
        if director.hero is not None:
            director.hero._gw_fx = None
            director.hero._gw_live_fx = False
    except Exception:                          # pragma: no cover
        pass
    director.clear()
...
def attach(hero):
    if not GRAVEWAKE_FX_ENABLED or hero is None:
        return False
    try:
        director_for(hero)
    except Exception:                          # pragma: no cover
...
def owns(hero):
    if not getattr(hero, "_gw_live_fx", False):
        return False
    d = getattr(hero, "_gw_fx", None)
...
def notify_melee_impact(hero, target, damage=0, crit=False):
    """Jangkar Gravewake mendarat di target (basic attack melee)."""
    if not GRAVEWAKE_FX_ENABLED or hero is None or target is None:
        return
    try:
...
def notify_projectile_impact(hero, x, y, angle=0.0, damage=0, crit=False,
                             kind="wave"):
    """Anchor wave / proc mengenai target."""
    if not GRAVEWAKE_FX_ENABLED or hero is None:
        return
    try:
...
def notify_skill_impact(hero, x, y, radius=None, skill="q"):
    """Skill Gravewake meledak di sebuah titik (Q/W/E/R AOE)."""
    if not GRAVEWAKE_FX_ENABLED or hero is None:
        return
    d = director_for(hero)
    ...
    d.skills.append(fx)
    kind = {"q": "wave", "w": "tide", "e": "shell",
            "r": "ravage"}.get(skill, "wave")
    d.on_impact(x, y, 0.0, 1.3 if skill != "r" else 2.0, skill == "r",
                kind=kind)


def notify_skill_cast(hero, skill):
    if not GRAVEWAKE_FX_ENABLED or hero is None or skill not in SKILL_DUR:
        return
    d = director_for(hero)
