# ============================================================================
# heroes/syrentha_fx.py
# ----------------------------------------------------------------------------
# Lapisan FX HIDUP untuk Syrentha "The Song of the Seas" (MINI BOSS level 6).
#
# Modul ini memegang SEMUA efek yang tidak boleh ikut ter-cache / ter-skala:
#   * partikel air (percikan, droplet, gelembung, busa, not musik)
#   * pita tusukan tombak (SwingTrail, dari histori posisi sungguhan)
#   * proyektil "Riptide Wave" (sabit air) untuk Q
...
#
# Arsitektur mengikuti heroes/thalgryn_fx.py dan heroes/kunkka_fx.py
...
#   * semua geometry/pose dibaca dari bosses/level6._NS_syrentha supaya
...
#: Master switch lapisan FX Syrentha.
SYRENTHA_FX_ENABLED = True
...
#: Jarak (piksel dunia) di bawah ini serangan dasar Syrentha dibaca sebagai
#: tusukan tombak yang menyambung ke target. = range di boss_data.py.
MELEE_REACH = 120

#: Durasi status skill (frame) - HARUS sinkron dengan bosses/base_boss.py
#: (_syrentha_q/w/e/r) dan hero_skills/_bundle.py.
...
WORLD_RADIUS = {"q": 250, "w": 150, "e": 150, "r": 220}
...
# ============================================================================
# 1.  PALETTE  (nilai cadangan; disinkronkan dari _NS_syrentha.PALETTE)
# ============================================================================
...
_PALETTE_SYNC = {
    ...
    "scale_darkest": "scale_darkest",
    "scale_dark": "scale_dark",
    "scale_mid": "scale_mid",
    "scale_light": "scale_light",
    "scale_high": "scale_high",
    "scale_shine": "scale_shine",
    "hair_darkest": "hair_darkest",
    "hair_dark": "hair_dark",
    "hair_mid": "hair_mid",
    ...
    "mirror_dark": "mirror_dark",
    "mirror_mid": "mirror_mid",
...
def _sync_palette():
    """Salin warna tema dari ``_NS_syrentha.PALETTE`` sekali saja."""
...
def _renderer():
    """``_NS_syrentha`` atau None. Diimpor malas (modul boss besar)."""
    global _RENDERER
    if _RENDERER is None:
        try:
            from bosses.level6 import _NS_syrentha as G
            _RENDERER = G
...
def _fallback_pose(boss):
    ...
    action = getattr(boss, "_sy_pose_action", None)
    if action is None:
        action = {"q": "riptide", "w": "song", "e": "mirror",
                  "r": "siren"}.get(skill)
    if action is None:
        action = ("attack" if getattr(boss, "_sy_attack_active", False)
                  else "idle")
    phase = float(getattr(boss, "pulse", 0.0) or 0.0)
    ap = 0.0
    if action == "attack":
        raw = float(getattr(boss, "_sy_attack_progress", 0.0) or 0.0)
...
def pose_of(boss):
    ...
    try:
        return G._resolve_pose(boss, bool(getattr(boss, "_sy_moving_cached",
                                                  False)))
    except Exception:                          # pragma: no cover
        return _fallback_pose(boss)
...
def screen_point(boss, x, y, local, action=None, phase=None, ap=None):
    """Ruang lokal rig -> piksel layar (sama dengan _NS_syrentha._local)."""
    G = _renderer()
...
def spear_points(boss, x, y):
    """(grip, tip) tombak dalam piksel layar — sumber bentuk trail."""
    G = _renderer()
    action, phase, ap = pose_of(boss)
    if G is None:
        grip_l = (15, 4)
...
# ============================================================================
# 6.  SWING TRAIL  —  jejak tombak dari histori posisi sungguhan
# ============================================================================

class SwingTrail:
    """Pita tusukan tombak berbasis histori (grip, tip) per frame."""
...
class ImpactFX:
    ...
    ``kind`` memilih bahasa visual:
        ``spear``      — tusukan tombak (garis baja + percikan air)
        ``riptide``    — Riptide (semburan horizontal air)
        ``song``       — Enchanting Song (cincin + not musik)
        ``mirror``     — Mirror Image (kabut biru + cincin ganda)
        ``siren``      — Song of the Siren (spiral + bintang stun)
    """

    KINDS = ("spear", "riptide", "song", "mirror", "siren")

    __slots__ = ("x", "y", "angle", "power", "age", "duration", "crit",
                 "active", "kind", "seed", "ground")

    def __init__(self, x, y, angle=0.0, power=1.0, crit=False,
                 kind="spear", ground=0.0, seed=None):
        self.x = float(x)
        self.y = float(y)
        ...
        self.crit = bool(crit)
        self.active = True
        self.kind = kind if kind in self.KINDS else "spear"
    ...
    def _ramp(self):
        if self.kind == "spear":
            return P["water_dark"], P["water_light"], P["blade_shine"]
        if self.kind == "riptide":
            return P["water_dark"], P["water_mid"], P["water_bright"]
        if self.kind == "song":
            return P["water_dark"], P["water_light"], P["water_hot"]
        if self.kind == "mirror":
            return P["mirror_dark"], P["mirror_light"], P["mirror_hot"]
        if self.kind == "siren":
    ...
    def draw(self, surface):
        ...
        if a > 6 and rr > 4:
            if self.kind == "spear":
                er = ellipse_ring_surface(rr, max(3, int(rr * 0.55)), th,
                                          mid, deg)
            elif self.kind in ("riptide", "song", "siren"):
            ...
            if rr2 > 4:
                er2 = ring_surface(rr2, max(1, th - 1), hot, 255,
                                   dashed=8 if self.kind != "spear" else 0)
        ...
        # 4. SLASH FRAGMENT (spear / riptide)
        if t < 0.52 and self.kind in ("spear", "riptide"):
            st = 1.0 - t / 0.52
            span = 0.5 + 0.5 * pw
        ...
        # 5. RETAKAN ZIGZAG (song / siren / mirror)
        if t < 0.62 and self.kind in ("song", "siren", "mirror"):
            st = 1.0 - t / 0.62
            n = 5 if self.kind != "siren" else 7
            for i in range(n):
        ...
        # 6. TETES AIR (mirror / siren)
        if self.kind in ("mirror", "siren") and t < 0.7:
            st = 1.0 - t / 0.7
            for i in range(6):
...
# ============================================================================
# 8.  PROJECTILE SYSTEM  —  RIPTIDE WAVE
# ============================================================================

class SyrenthaProjectile:
    """Riptide Wave (Q) — sabit air yang melaju ke depan.
    ...
    def draw(self, surface):
        ...
        for i, pv in enumerate(self.trail):
            ...
            _shard_poly(surface, pv.x, pv.y, self.rotation, 14, 3,
                        P["water_dark"], a)
        draw_riptide_wave(surface, self.position.x, self.position.y,
...
class ProjectileSystem:
    ...
    def spawn(self, x, y, tx, ty, **kw):
        ...
        pr = SyrenthaProjectile(x, y, tx, ty, **kw)
...
class SkillFX:
    """Efek skill Syrentha dengan lifecycle bertahap.
    ...
    TINT = {
        "q": ((12, 62, 96), (150, 226, 248)),     # Riptide: arus air
        "w": ((16, 60, 110), (210, 248, 252)),    # Song: air + cahaya
        "e": ((30, 90, 130), (200, 240, 250)),    # Mirror: biru pucat
        "r": ((10, 52, 96), (255, 230, 90)),      # Siren: putih + kuning
    }
        ...
        if self.phase == "charge":
            ...
            if kind == "e":
                ps.burst(self.x, self.y + self.ground, 4,
                         ...
                         colors=(P["mirror_light"], P["mirror_mid"], tint_d),
        ...
        if self.phase in ("release", "area") and not self._released:
            ...
            if kind == "q":
                ...
                ps.stream(self.x, self.y,
                          ...
                          colors=(P["mirror_light"], P["mirror_mid"],
                                  P["mirror_dark"]), bow=20.0, layer="back")
            elif kind == "w":
    ...
    def draw_front(self, surface):
        ...
        if self.kind == "e" and self.phase in ("release", "area"):
            ...
            for i, (ox, oy) in enumerate(((-55, 18), (55, 18), (-32, -24))):
                ...
                _draw_mirror_silhouette_fx(surface, mx, my, self.facing,
                                           a + i, alpha)

        # ── R: spiral not + bintang stun ─────────────────────────────
...
def _draw_music_note_fx(surface, cx, cy, phase, alpha=220):
    alpha = max(0, min(255, int(alpha)))
    head_r = 3
    pygame.draw.circle(surface, (*P["water_dark"], alpha), (cx, cy),
                       head_r + 1)
    pygame.draw.circle(surface, (*P["water_mid"], alpha), (cx, cy), head_r)
    pygame.draw.circle(surface, (*P["water_light"], alpha), (cx - 1, cy - 1),
                       max(1, head_r - 1))
    pygame.draw.circle(surface, (*P["water_bright"], alpha),
                       (cx - 1, cy - 1), max(1, head_r - 2))
    stem_h = 10
    pygame.draw.line(surface, (*P["water_dark"], alpha),
                     (cx + head_r, cy), (cx + head_r, cy - stem_h), 2)
    pygame.draw.line(surface, (*P["water_mid"], alpha),
                     (cx + head_r, cy), (cx + head_r, cy - stem_h), 1)
    pygame.draw.polygon(surface, (*P["water_mid"], alpha), [
        (cx + head_r, cy - stem_h), (cx + head_r + 4, cy - stem_h + 3),
        (cx + head_r + 3, cy - stem_h + 6), (cx + head_r, cy - stem_h + 4)])
...
def _draw_mirror_silhouette_fx(surface, cx, cy, facing, phase, alpha=200):
    """Siluet air ethereal mini untuk SkillFX E."""
    alpha = max(0, min(255, int(alpha)))
    if alpha <= 10:
        return
    pygame.draw.circle(surface, (*P["mirror_dark"], alpha // 3),
                       (cx, cy), 34)
    pygame.draw.circle(surface, (*P["mirror_dark"], alpha // 2),
                       (cx, cy), 24)
    pygame.draw.polygon(surface, (*P["mirror_dark"], alpha), [
        (cx - 11, cy - 5), (cx + 11, cy - 5), (cx + 13, cy + 5),
        (cx + 9, cy + 19), (cx + 4, cy + 28), (cx - 4, cy + 28),
        (cx - 9, cy + 19), (cx - 13, cy + 5)])
    pygame.draw.polygon(surface, (*P["mirror_mid"], alpha), [
        (cx - 9, cy - 4), (cx + 9, cy - 4), (cx + 11, cy + 5),
        (cx + 7, cy + 17), (cx + 3, cy + 25), (cx - 3, cy + 25),
        (cx - 7, cy + 17), (cx - 11, cy + 5)])
    pygame.draw.polygon(surface, (*P["mirror_mid"], alpha), [
        (cx - 3, cy + 28), (cx + 3, cy + 28), (cx + 9, cy + 36),
        (cx + 4, cy + 40), (cx, cy + 36), (cx - 4, cy + 40),
        (cx - 9, cy + 36)])
    pygame.draw.polygon(surface, (*P["mirror_dark"], alpha), [
        (cx - 9, cy - 14), (cx + 9, cy - 14),
        (cx + 11, cy - 5), (cx - 11, cy - 5)])
    pygame.draw.circle(surface, (*P["mirror_dark"], alpha),
                       (cx, cy - 21), 8)
    pygame.draw.circle(surface, (*P["mirror_mid"], alpha),
                       (cx - 1, cy - 22), 7)
    pygame.draw.circle(surface, (*P["mirror_light"], alpha),
                       (cx - 2, cy - 23), 4)
    for x_off, y_off in [(0, -34), (-5, -31), (5, -31)]:
        pygame.draw.polygon(surface, (*P["mirror_mid"], alpha), [
            (cx + x_off // 2 - 3, cy - 26), (cx + x_off // 2 + 3, cy - 26),
            (cx + x_off, cy + y_off)])
    pygame.draw.circle(surface, (*P["mirror_hot"], alpha), (cx - 3, cy - 21), 1)
    pygame.draw.circle(surface, (*P["mirror_hot"], alpha), (cx + 3, cy - 21), 1)


# ============================================================================
# 10. DIRECTOR — mengikat semua sistem FX untuk satu unit Syrentha
# ============================================================================
...
class SyrenthaFXDirector:
    """Mengikat particle + trail + proyektil + impact + skill FX untuk
    satu unit Syrentha (mini boss; kodenama sama untuk jalur hero)."""
    ...
    def on_swing_impact_frame(self, x, y, facing):
        """Tusukan tombak di frame IMPACT (connect melee ke target)."""
        _g, tip = spear_points(self.hero, x, y)
        ...
        if tgt is not None and getattr(tgt, "alive", False):
            ...
            if d <= MELEE_REACH:
                ...
                melee = True
        if melee:
            self.on_impact(tx, ty, ang, 1.0, False, kind="spear")
        else:
            self.impacts.append(ImpactFX(tip.x, tip.y, ang, 0.5, False,
                                         kind="spear", ground=0.0,
    ...
    def _resolve_state(self):
        ...
        if getattr(h, "_sy_attack_active", False):
            ph = attack_phase(float(getattr(h, "_sy_attack_progress", 0.0)))
            if ph in ("ANTICIPATION", "WINDUP"):
                return "CHARGE"
            if ph in ("SWING", "IMPACT"):
                return "SWING"
            return "ATTACK"
        if getattr(h, "_sy_moving_cached", False):
            return "RUN" if float(getattr(h, "speed", 1.0)) >= 2.2 \
                else "WALK"
        st = getattr(h, "_sy_state", None)
    ...
    def _update_state(self, dt):
        ...
        if want != self.state:
            ...
            if self.state != "DEATH" and (new_p >= cur_p
                ...
                self.state_time = 0.0
        self.state_time += dt
        self.anim_phase = str(getattr(self.hero, "_sy_attack_phase", None)
                              or "NONE")
    ...
    def update(self, dt, x, y):
        ...
        attacking = bool(getattr(h, "_sy_attack_active", False))
...
def should_freeze_frame():
    if not SYRENTHA_FX_ENABLED or _feel is None:
        return False
...
# ============================================================================
# 12.  RIPTIDE WAVE RENDERER BERSAMA
# ============================================================================

def draw_riptide_wave(surface, px, py, angle=0.0, age=0, radius=26.0,
                      seed=0):
    """Bulan sabit air (Riptide Wave): core + glow + busur terarah +
    sparkle. Digambar langsung (tanpa transform.rotate)."""
    px, py = int(px), int(py)
...
def draw_debug_overlay(surface, director):
    ...
    try:
        ...
        x = int(getattr(h, "x", 0))
        y = int(getattr(h, "y", 0))
        r = max(6, int(getattr(h, "radius", 38) * 0.9))
        pygame.draw.rect(surface, (80, 170, 255, 150),
                         pygame.Rect(x - r, y - r - 8, r * 2, r * 2), 1)
        rng = max(10, int(getattr(h, "range", 120) * 0.9))
...
def director_for(hero):
    """Ambil (atau buat) director FX untuk satu unit Syrentha."""
    _sync_palette()
    d = getattr(hero, "_sy_fx", None)
    if d is None:
        d = SyrenthaFXDirector(hero)
        try:
            hero._sy_fx = d
        except Exception:                      # pragma: no cover
            return d
...
def attach(hero):
    if not SYRENTHA_FX_ENABLED or hero is None:
        return False
    try:
        director_for(hero)
    except Exception:                          # pragma: no cover
...
def notify_melee_impact(hero, target, damage=0, crit=False):
    """Tombak Syrentha mendarat di target (basic attack melee)."""
    if not SYRENTHA_FX_ENABLED or hero is None or target is None:
        return
    try:
...
def notify_projectile_impact(hero, x, y, angle=0.0, damage=0, crit=False,
                             kind="riptide"):
    """Riptide wave / proc mengenai target."""
    if not SYRENTHA_FX_ENABLED or hero is None:
        return
    try:
...
def notify_skill_impact(hero, x, y, radius=None, skill="q"):
    """Skill Syrentha meledak di sebuah titik (Q/W/E/R AOE)."""
    if not SYRENTHA_FX_ENABLED or hero is None:
        return
    d = director_for(hero)
    ...
    d.skills.append(fx)
    kind = {"q": "riptide", "w": "song", "e": "mirror",
            "r": "siren"}.get(skill, "riptide")
    d.on_impact(x, y, 0.0, 1.3 if skill != "r" else 2.0, skill == "r",
                kind=kind)


def notify_skill_cast(hero, skill):
    if not SYRENTHA_FX_ENABLED or hero is None or skill not in SKILL_DUR:
        return
    d = director_for(hero)
