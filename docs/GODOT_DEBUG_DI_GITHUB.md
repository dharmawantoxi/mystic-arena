...
| `preflight.log` + `marker.txt` | uji SEBELUM run sungguhan, dua bagian: (1) engine mengompilasi skrip debug lewat `--check-only -s` — menangkap API yang tidak ada di versi engine (mis. 4.4 di engine 4.3), (2) engine menjalankan `scenes/debug/DebugMarker.tscn` (tanpa satu pun API game) yang menulis berkas. Lulus = skrip+engine+scene jalan; gagal = masalahnya di skrip debug/engine/project, bukan di skenario |
...
| `summary.md` menulis **harness TIDAK PERNAH JALAN** | skrip debug tidak bisa dikompilasi engine yang dipakai (paling sering: API yang baru ada di 4.4 dipanggil di engine 4.3), atau scene tidak dijalankan engine | baca dulu `preflight.log` bagian (1): di situ skrip debug dikompilasi dan hasilnya disebut per berkas. Kalau preflight lulus tetapi run tetap sunyi, cari `Parse Error`/`SCRIPT ERROR` di `run.log` dan periksa baris `$ …` pertama. Kunci otomatisnya: `tools/test_godot_debug_runner.py` (cek `--check-only` + larangan API 4.4+) |
| `summary.md` menulis **preflight GAGAL** | (a) skrip debug memakai API yang tidak ada di versi engine (mis. `get_current_rendering_method` = 4.4, di CI 4.3), atau (b) engine tidak bisa menjalankan bahkan scene penanda tanpa API game | lihat `preflight.log`: bagian (1) menyebut skrip mana + polanya. Perbaiki dengan helper `has_method`/`call` (lihat `DebugProbe._server_field`) atau jalankan `godot_version=4.7.2`. Untuk (b): `Parse Error` di skrip penanda, project tidak termuat, atau dependensi GL/X11 tidak ada |
| Run gagal hanya di satu versi engine (`4.3` vs `4.7.2`) | project mendukung 4.3, jadi API 4.4+ belum boleh dipakai langsung | `godot/scenes/debug/*.gd` hanya memakai API 4.3; lingkungan baru = lewat `has_method(...)` + `call(...)`. Gate `tools/test_godot_debug_runner.py` menolak panggilan langsung ke `RenderingServer.get_current_rendering_*` |
