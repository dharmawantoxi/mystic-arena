...
| Multi-slot save + migrasi legacy lokal (FASE 20C) | Port Godot hanya memiliki satu `mystic_save.json`; `progress.json`, metadata slot, create/load/delete, dan `level_stats` lintas slot belum tersedia. Cloud/plugin Android sengaja dipisahkan sebagai subfase. | `SaveManager.gd` kini memiliki 3 slot `user://slot_1.json`…`slot_3.json`, metadata slot, `create` implisit saat save pertama, `set_current_slot`/load/delete, migrasi insert-only `progress.json` → Slot 1 dengan backup `.old`, serta `level_stats` di working copy. `MainMenu.gd` memiliki layar SLOT_SELECT sebelum LEVEL_SELECT. Oracle `save_slots` + `save_slot_delete` memakai filesystem Pygame terisolasi, dua seed identik, dan replay `ProgressionParityTest`; cloud payload checksum lokal teruji, tetapi adapter/plugin Google Play Android tetap subfase tersendiri. |
...
- **Progresi/settings:** kunci difficulty sepanjang run, reset progresi karena
  ...
  playtime, dan `level_stats` belum diport~~ — **DITUTUP FASE 20C** untuk
  filesystem lokal: tiga slot, metadata, create/load/delete, migrasi
  `progress.json` ke Slot 1 dengan backup, serta `level_stats` direplay lewat
  `ProgressionParityTest` memakai filesystem terisolasi. Payload cloud +
  checksum lokal ikut diuji; adapter/plugin Google Play Android tetap
  subfase terpisah dan belum diklaim;
  ...
  **DITUTUP FASE 20**: oracle `meta_shop_txn` mengunci katalog EFEKTIF
  ...
  disengaja; (5) adapter/plugin cloud Google Play Android dan sinkronisasi
  snapshot lintas-perangkat masih terbuka; payload lokal + checksum sudah
  memiliki kontrak dan tes, tetapi tidak diklaim sebagai integrasi Android.
...
godot --headless --path godot res://tests/ProgressionParityTest.tscn --quit-after 300
...
`ProgressionParityTest` memutar ulang seksi insert-only `progression_level_select_stats`,
`hero_shop_meta`, `save_slots`, `save_slot_delete`, dan `phase20_determinism` pada
`SaveManager.gd`/`MainMenu.gd` produksi. Ia membuat tiga slot, membaca metadata,
memilih/memuat slot, menghapus slot, memigrasikan `progress.json` hanya bila Slot 1
kosong, memeriksa backup legacy, dan memvalidasi payload cloud lokal dengan checksum.
Timestamp wall-clock dibandingkan sebagai presence flag; filesystem `user://` selalu
diisolasi oleh CI. `mobile/cloud_save.py`/plugin Google Play Android tetap subfase
terpisah — payload lokal bukan klaim sinkronisasi cloud lintas-perangkat.
