...
| Verifikasi | (1) `gen_ui_cpp.py --check` di **kedua** workflow (3 berkas byte-identik); (2) `test_ui_cpp_selftest.py` — MENG-EXECUTE `ui_processor.cpp` apa adanya lewat stub Variant tanpa engine: **205 cek** vs fixture `ui_hud` (angka direkam dari draw pygame ASLI) + `_core.py`/`ui_theme.py` + built-in Python (`round()` half-to-even, `f"{v:,}"`), **closed-world** (tabel perintah == `bind_static_method` == deklarasi `.h`; setiap fungsi ter-bind wajib diuji), **audit wiring** (entry symbol, `.gdextension`, allowlist SEMPIT `godot_log_gate`, rujukan kedua workflow, `.gitignore`), dan **opsional cek sintaks vs header godot-cpp asli** (`GODOT_CPP_DIR`) — penangkap kelas bug yang lolos dari stub: `color.r8()` vs `color.get_r8()` (kejadian nyata CI PR #234); (3) build scons + `nm -D` (`mystic_ui_library_init` + symbol `MysticUI`) di `godot-gdext.yml` |
