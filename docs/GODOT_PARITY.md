...
| Verifikasi | (1) `gen_ui_cpp.py --check` di **kedua** workflow (3 berkas byte-identik); (2) `test_ui_cpp_selftest.py` — MENG-EXECUTE `ui_processor.cpp` apa adanya lewat stub Variant tanpa engine: **210 cek** (206 tanpa checkout godot-cpp) vs fixture `ui_hud` (angka direkam dari draw pygame ASLI) + `_core.py`/`ui_theme.py` + built-in Python (`round()` half-to-even, `f"{v:,}"`), **closed-world** (tabel perintah == `bind_static_method` == deklarasi `.h`; setiap fungsi ter-bind wajib diuji), **audit wiring** (entry symbol, `.gdextension`, allowlist SEMPIT `godot_log_gate`, rujukan kedua workflow, `.gitignore`), dan **opsional cek sintaks vs header godot-cpp asli + compile/link .so probe** (`GODOT_CPP_DIR`; `-fvisibility=hidden` = default godot-cpp) — penangkap kelas bug yang lolos dari stub: `color.r8()` vs `color.get_r8()` dan cek symbol kelas yang MUSTAHIL cocok (`nm -D`, padahal visibility hidden), keduanya kejadian nyata CI PR #234, kini ketahuan lokal dalam ±10 detik alih-alih menunggu siklus CI ±30 menit; (3) build scons + cek symbol di `godot-gdext.yml` (`nm -D` untuk entry `mystic_ui_library_init`; `nm -C` statis + demangle `godot::MysticUI::` untuk kelasnya — godot-cpp default `symbols_visibility=hidden`, jadi `nm -D` MUSTAHIL memuat symbol kelas dan pernah bikin langkah build gagal palsu) |
