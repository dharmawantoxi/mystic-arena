...
- **UI/HUD/toko/menu:** perilaku in-match kini diuji oracle (`ui_hud` +
  ...
  Godot membuka toko via H); (4) ~~**pemicu UI taktis**~~ (**DITUTUP
...
- **Perintah taktis dan kontrol pemain:** sistem `tactical_commands.py` dan
  pemicunya sudah ditutup FASE 17–18 (`TacticalCommandsParityTest` +
  `TacticalInputParityTest`). Yang masih terbuka di klaster ini hanya kontrol
  sentuh Android penuh dan audit piksel/audio UI taktis; state keyboard/panel
  produksi sudah dikunci.
- **Progresi/settings:** kunci difficulty sepanjang run, reset progresi karena
  ganti mode, statistik/achievement, semua pilihan settings dan transport cloud
  Android belum seluruhnya setara. Tidak menghapus save pengguna untuk
  menyamarkan selisih. Yang SUDAH setara + teruji dari blok ini termasuk
  sumber jumlah unlock catch-up (`hero_catchup_unlocks`/`HeroCatchupUnlockParityTest`),
  reward/atribusi/unlock kematian boss (FASE 14), dan seluruh transaksi/state
  berikut yang ditutup FASE 20 oleh `ProgressionParityTest`:
  (1) **BEST SCORE/best time per level** di LEVEL SELECT (`_core.py:4257-4271`
  + `SaveManager.format_time`) — `level_stats` dibaca dari slot aktif dan
  format waktu `--:--`/`M:SS` serta win rate dikunci oracle;
  (2) **alur pembelian hero di Hero Shop meta** (`_unlock_hero_in_meta_shop`:
  katalog, duplicate, `unlock_require_boss`, saldo, harga 4500, potong gold,
  dan append unlock) — Godot menulis satu transaksi atomik dan oracle
  `hero_shop_meta` menguji success/reject tanpa efek parsial;
  (3) **multi-slot + migrasi legacy lokal** — tiga slot, `get_slot_info`,
  playtime, level stats, select/delete, dan `progress.json` → `slot_1.json`
  dengan `progress_backup.json.old` kini diputar ulang dari filesystem
  `user://` terisolasi. Payload cloud Godot sudah punya magic/version/checksum
  dan restore contract, tetapi integrasi transport Google Play Games/plugin
  Android tetap terbuka dan tidak disertifikasi headless;
  (4) penempatan auto-grant starter masih berbeda: Godot memberi `kaizen`
  saat save di-backfill/load, pygame saat `Game.reset`. Jumlah unlock catch-up
  sama-sama 0, tetapi status OWNED pada save baru muncul lebih awal di Godot —
  belum disamakan karena menyentuh alur save.
...
godot --headless --path godot res://tests/ProgressionParityTest.tscn --quit-after 240
...
`ProgressionParityTest` memutar ulang tiga seksi FASE 20 pada sistem produksi:
`MainMenu.level_stat_display()` untuk statistik kartu level; `MainMenu.hero_shop_transaction()`
+ `SaveManager.commit_hero_purchase()` untuk validasi transaksi meta; dan
`SaveManager` untuk metadata tiga slot, pemilihan, migrasi legacy, serta payload
checksum cloud. Timestamp dinormalisasi menjadi flag keberadaan karena wall-clock
bukan state gameplay. Fixture dijalankan dua kali dengan seed `20260920` dan
`42420`; jalur ini sengaja tidak mengonsumsi RNG dan kedua hasil wajib identik.
Tidak ada perbandingan piksel/audio.
...
- **FASE 20 — statistik level, Hero Shop meta, dan multi-slot save:** oracle
  freshness lulus dengan `XDG_DATA_HOME=$(mktemp -d)` dan fixture insert-only;
  seksi baru `level_select_stats` (5 kasus formatter/view state), `hero_shop_meta`
  (7 transaksi success/reject), dan `save_slots` (5 kasus slot/migrasi) dibuat
  dari fungsi Pygame asli. Dua seed berbeda identik, timestamp dinormalisasi,
  dan semua seksi fixture sebelum FASE 20 byte-identik. Replay produksi
  `ProgressionParityTest` menutup view model LEVEL SELECT, transaksi atomik
  Hero Shop, tiga slot, backup legacy, serta checksum/penolakan payload cloud.
  Tidak ada perubahan runtime pygame dan tidak ada klaim pixel/audio parity.
  Transport Google Play Games tetap dibatasi pada serializer/restore contract
  sampai plugin Android tersedia.
