...
| Verifikasi | Hanya bagian **PALET** di `tools/visual_parity_audit.py` (konstanta warna) | `tools/test_godot_ui_theme_parity.py`: 4 cek statik (PALET 32/32 dua arah, COVERAGE 31 def → 617 simbol closed-world, IKON 29 + lengan `match`, LITERAL 29 warna, WIRING 17 seksi) **plus** menjalankan `ui_theme.py` ASLI (SDL dummy + font palsu 7px/karakter) untuk merekam fixture 18 seksi/66 KB; `godot/tests/UiThemeParityTest.tscn` memutar ulangnya di engine; CI `godot-check` langkah **4y** + `ui_theme.py` masuk filter `paths` |
