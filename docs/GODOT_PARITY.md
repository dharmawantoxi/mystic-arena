...
partikel + latar gradien/vignette + glow judul + hint skip, auto-selesai 3.0
detik, bisa di-skip) sudah punya port Godot (`scenes/ui/SplashScreen.gd`) yang
**tidak pernah dibandingkan** dengan pygame-nya — dan ternyata menyimpang di
dua tempat yang langsung terlihat pemain: latarnya gradien ungu tipis padahal
pygame menghasilkan interior **hitam pekat + rim 2 px**, dan skip-nya memakai
timer sejak tombol ditekan padahal pygame mengukur **elapsed total**. Fase ini
memulai dari oracle (MENJALANKAN pygame aslinya), lalu menambahkan lapisan
GDScript + **lapisan C++** (`MysticSplash`, GDExtension `mystic_splash`,
**62 method static** dibangkitkan dari AST), saklar backend, rewrite renderer,
dan empat lapis verifikasi. Rincian teknis + semantik Python↔Godot:
[SPLASH_GODOTPP.md](SPLASH_GODOTPP.md).

| Bagian | Sebelum (FASE 38) | Sesudah |
|---|---|---|
| Sumber angka splash | Satu jalur: `SplashScreen.gd` menulis ulang konstanta + rumus pygame dengan tangan (durasi, fade, ambang judul, gerak partikel, 48 batang gradien, vignette, geometri logo, glow judul, hint) dan tidak ada pembanding | Tiga lapisan dari satu sumber: Python (oracle) → GDScript (`scripts/ui/SplashModel.gd`, 480 baris: 62 fungsi + konstanta + `round_half_even`/`py_int`/`API_SIGNATURE`) → **C++** (`gdext/mystic_splash/src/splash_processor.h` 108 baris + `.cpp` 715 baris, 62 method static `MysticSplash : RefCounted` + `selftest/splash_dispatch.inc` 255 baris). `SplashScreen.gd` (279 baris) tinggal renderer: setiap angka lewat `SplashBackend` |
| Latar (gradien + vignette) | **Deviasi produksi**: gradien ungu + vignette tipis. Loop vignette pygame `for i in range(140, 0, -2)` dengan `alpha = min(255, int(2.2*(140-i)))` MENJENUH ke 255 pada 12 bingkai terakhir (`i <= 24`), dan karena digambar luar→dalam lalu di-blit, interior layar **hitam pekat** dengan rim 2 px yang menampakkan gradien | `bg_bands()` (48 batang, `step_h = max(1, h//48)`, split 0.55, `int()` per kanal) + `vignette_frames()` (70 bingkai + alpha) digambar apa adanya oleh renderer. Dikunci fixture **9 probe piksel hasil render pygame asli** (`(0,0)`/`(1,1)` = `[8,8,18]`, interior `[0,0,0]`, `(1279,719)` = `[6,6,15]`) + kompositor di kedua scene engine. Latar tidak dimodulasi alpha (`blit` opaque, `:230`); gerbangnya `draw_visible(a)` = `a > 0.001` (`:180`) |
| Skip & selesai | **Deviasi produksi**: `_skip_t` (timer sejak skip) → skip di `t = 2.0` masih memudar 0.25 detik, padahal pygame langsung selesai | `skip()` pygame hanya menyetel bendera (`:125-128`); `done` dihitung `update()` **frame berikutnya** dengan `elapsed >= 0.25` **TOTAL** (`:148-155`) dan `_overall_alpha()` memakai `max(0, 1 - t/0.25)` (`:167-176`). Renderer kini memanggil `is_done`/`overall_alpha` (semantik pygame); semantik port lama dipindah ke `is_done_godot`/`overall_alpha_godot` (argumen `skip_t`) supaya A/B harness bisa menunjuk bedanya. Fixture menyimpan **dua bendera skip per baris** (saat `update()` dan saat alpha direkam) + `elapsed` **presisi penuh** (frame 180 = `2.9999999999999996 < 3.0`, jadi `done` baru `True` di frame 181 — pembulatan 9 digit membalik ambang ini dan memang sempat bikin self-test merah) |
| Partikel + RNG | 46 partikel `randf()` tanpa seed dan gerak ditulis tangan di `_process`; tidak ada pembanding | Gerak **per FRAME** (`y -= speed`, `x += drift + sin(t*0.8+phase)*0.05`, `:139-146`) lewat `particle_advance`, wrap `< -6` strict → `h + 6`, dan `respawn_x` jadi **argumen** karena `random.uniform` pygame **tak ber-seed** (tidak ada stream yang bisa diklaim — deviasi disengaja: Godot memakai seed tetap 777). Paritas RANTAI tetap diuji: oracle me-spy `random.uniform` sehingga nilai respawn pygame terekam di fixture, lalu 46 partikel × 120 langkah dibandingkan bit-dekat (tol `1e-9`) di self-test C++ **dan** kedua scene engine. State partikel = `Dictionary` of double, **bukan `Vector2`** (float 32-bit cukup untuk menggeser frame wrap) |
| Glow logo + judul | Cincin glow logo dan lapisan glow judul ditulis tangan; `range(glow_r, 0, -3)` pygame pernah ditranskripsi sebagai langkah **+3** sehingga `logo_glow_rings` C++ **loop tak berujung** (self-test kehabisan memori, `rc = -9`) | `logo_scale`/`logo_base_size`/`logo_grow_quant` (`round()` **half-to-even** × 20 / 20, `:278`)/`logo_glow_radius`/`logo_glow_rect`/`logo_glow_rings` (langkah `-3` bagian dari spec, 64 cincin untuk `glow_r = 190`, dikunci menurun di kedua scene) + `title_glow_layers` (`[(24,1),(14,2),(8,3)]` hanya saat `cheap_alpha`, `:323-327`)/`title_glow_surface`/`title_glow_fill` (`int(alpha*0.28)`, `BLEND_RGBA_MULT`)/`accent_gap` (`w//2 + 18`)/`accent_lines` (off ±6, `fac = (alpha/255)*0.85`, panjang 46, lebar 2). **Metrik font tidak deterministik** → oracle memakai font palsu (`w = len*size//2`) dan sisi Godot mengirim **lebar teks terukur** sebagai argumen (pola FASE 31) |
| Pemilihan backend | Tidak ada | `scripts/ui/SplashBackend.gd` (522 baris): `mystic/splash/use_gdext_splash` + `ClassDB.class_exists("MysticSplash")`, instance di-cache, `force_backend()`/`reset_backend()`/`backend_name()`/`gdext_available()`, log sekali per perubahan backend (`GDExtension MysticSplash aktif` — literal yang di-require CI). Tidak pernah menyebut class GDExt sebagai identifier (di CI headless lib tidak ikut repo → Parse Error mematikan seluruh project) |
| Verifikasi | Tidak ada: splash Godot tidak pernah dibandingkan dengan pygame | (1) `gen_splash_cpp.py --check` di **kedua** workflow (3 berkas byte-identik); (2) `test_godot_splash_parity.py` — **613 cek**, MENJALANKAN `SplashScreen` pygame ASLI (SDL dummy + font palsu + `TrackSurface` sebagai `screen`, karena `pygame.Surface.blit` tidak bisa di-monkeypatch) untuk merekam fixture 141 KB: timeline 4 skenario × 200 frame, rantai partikel, latar + probe piksel, logo, glow judul, hint, dan **urutan blit/draw** 2 skenario; lalu membandingkannya dengan `SplashModel.gd` + literal C++, mengaudit **API tiga sisi** (bind == routing backend == deklarasi `.h`) dan **arity tiap panggilan `Backend.*`** di renderer + kedua scene (gdparse hanya memeriksa sintaks; Godot memeriksa arity lintas berkas saat scene dimuat), plus **ruang lingkup identifier GDScript** (nama `ALL_CAPS` telanjang di lima berkas splash `.gd` harus terdeklarasi di berkas itu sendiri atau ada di daftar global engine eksplisit — salah ketik `GROW_GROW` vs `GLOW_GROW` lolos `gdparse` dan baru meledak di CI sebagai `Parse Error: Identifier ... not declared` yang menyeret `SplashScreen.gd` + `Main.gd`), **semantik argumen sweep** (`bg_band_rect`/`bg_band_color` menerima y piksel, bukan indeks batang — arity sah tapi 46/48 batang sempat salah warna di CI), wiring closed-world, dan kesegaran fixture; (3) `test_splash_cpp_selftest.py` — **MENG-EXECUTE** `splash_processor.cpp` apa adanya lewat stub `Variant`: **1.585 cek (1.580 tanpa checkout godot-cpp) / 6.524 perintah** vs fixture yang sama, closed-world (tabel perintah == `bind_static_method` == deklarasi `.h`; setiap fungsi ter-bind diuji, termasuk `api_signature`), audit wiring, dan — kalau ada checkout godot-cpp — `-fsyntax-only` terhadap header **asli** + compile/link/**strip** `.so` probe lalu ulangi cek langkah build; (4) build scons + `nm -D` entry `mystic_splash_library_init` + string tahan-strip (`splash_v1:1mod:62fn:`, `MysticSplash`); (5) `SplashParityTest` (backend GDScript + smoke scene `SplashScreen` asli: fade in → `skip()` → signal `finished`) + `SplashGdextParityTest` (paksa C++, **gagal keras** kalau lib tidak ada, plus A/B `str(Variant)` seluruh permukaan API — ikut mengunci urutan kunci `Dictionary`, tipe, dan presisi double) |
| CI GDExt | Lima lib (`mystic_skills`, `mystic_levels`, `mystic_maps`, `mystic_ui`, `mystic_mobile`) | Enam lib: symlink godot-cpp bersama + build + `nm`/`strings` `mystic_splash`, langkah self-test splash **SEBELUM** build godot-cpp yang ±10 menit, `timeout-minutes` job **45 → 75** (enam lib × ±8-10 menit build melewati cap lama: run pertama FASE 38 dibatalkan GitHub di menit 45 saat `Build mystic_mobile` baru berjalan — kehabisan waktu, bukan kegagalan kode), scene GDExt (require `PASS` + `GDExtension MysticSplash aktif`) dan regresi "lib terpasang tapi flag `false` → tetap GDScript". `godot_log_gate.py` diberi tiga entri allowlist **sempit** (`libmystic_splash`, `addons/mystic_splash`, `Failed loading resource.*mystic_splash`) supaya baris FAIL harness tetap fatal |

**Yang TIDAK berubah (sengaja):** `mystic/splash/use_gdext_splash` tetap
`false` — jalur produksi masih GDScript (`SplashModel.gd`), sama seperti FASE
33-36: C++ paritas-teruji di CI, tapi menyalakannya untuk pemain berarti
mewajibkan lib per platform (dua arch Android + wasm) di setiap rilis.
`build-android-godot.yml` belum memanggil scons; CI hanya membuild linux x86_64.
RNG partikel **tidak** diklaim paritas (pygame tak ber-seed) — Godot memakai
seed tetap 777, yang diklaim adalah rantai dengan nilai respawn terekam dan
distribusi rentang/warna. Piksel glow (cincin logo `draw_arc` lebar 3 px karena
pygame MENIMPA disk bertingkat, lapisan glow judul `UiTheme.draw_glow`) tetap
pendekatan renderer; angka geometrinya dari backend dan dikunci fixture.
`splash_screen.py` **tidak disunting** (`docs/MIGRASI_1_1.md`).
...
| Kaizen | Rig buatan ulang selalu mengalahkan sprite Pygame | Sejak 2026-09-15 rig v4 menjadi **default** arena (`experimental_hero_rigs=true`, dikunci `GameplayParityTest` bersama fallback bake saat flag false). Showcase isolasi di `KaizenDemo.tscn`. |
