# Morgath v2 — Renderer Pixel-Art Masterwork + v2.1 Skill FX + v2.3 Bolt
...
## Basic attack — Lightning Bolt v2.3 (mewah)

Beam dasar bukan lagi garis putus-putus sederhana. `_draw_lightning_
projectile` di-rewrite penuh dengan **11 lapis elemen**, semuanya
prosedural & deterministik per-frame (belum ada PNG apa pun):

1. **Bloom radial** — sprite glow 4-band dibangun sekali via
   `_static()` per ukuran (cache miss saja), di-blit dengan
   `set_alpha` + restore, **terpusat presisi** di kepala & telapak
   (fix bug offset v2.2 yang melenceng 12 px).
2. **Ghost chord** — echo samar seluruh bentuk chord di-offset tegak
   lurus (dobel-eksposur listrik).
3. **Echo arc** — 2 busur petir melompat keluar lalu *menyambar
   balik* ke jalur utama (leader loop), sisi selang-seling per-frame.
4. **Chord utama tepi-ke-tepi** — polyline bertekuk di tengah tiap
   segmen; tekukan morph hidup tiap frame (2 oktaf sinus + pulse),
   3 lapis pita (arc_dark → arc_mid → arc_light) dengan lebar menirus
   5→1 px menuju kepala + offset heliks antar lapis, inti arc_hot
   menyala di 40% ujung dekat kepala.
5. **Pulse energi berjalan** — pita putih (arc_shine + arc_hot)
   menyusul kepala 1.6× lebih cepat, plus titik putih 2 px.
6. **Ranting letik menyimpang** — 3 cabang `_jagged_line` 2-lapis.
7. **Mach rings** — 3 cincin hollow membesar makin jauh di belakang
   kepala (wake kecepatan).
8. **Trail after-image hollow + mote bara** — 3 ghost ring + 7 mote
   2-warna berekor berjatuhan dari lintasan.
9. **Kepala bolt** — flare 5 lapis + inti putih + **corona 6 paku
   berputar** (2 jagged + 4 lurus, ujung menyala di luar disk flare)
   + 2 paku cahaya silang + glint orbit 3 titik.
10. **Percik las di telapak** — 3 busur kecil berkedip sepanjang
    flight + bintang pelepasan saat bolt lahir (t < 0.25).
11. **Benturan berlapis** — scorch glow + ring ganda + **ring tunda**
    (micro-ring menyusul) + bintang 8-spike + 8 garis radial + 4 fork
    jagged + 8 serpihan berekor (t > 0.88).
...
~1.0 ms untuk frame impact penuh (jauh di bawah 3.5 ms).
...
- `tools/_audit_morgath_v2.py` — audit terukur: ukuran layar final,
  ...
  canvas, before/after vs snapshot v1, plus **blok bolt v2.3**
  (lahir dari telapak, 5 band arc, kekayaan di luar sumbu, bloom
  terpusat, echo arc, pulse berjalan, mach rings, corona 6 paku,
  percik las, 8 frame morph unik, impact berlapis, budget ≤ 3.5 ms).
...
- Preview: `docs/morgath_v2_review.png`, `morgath_v2_anim_strip.png`,
  ...
  `morgath_v2_before_after.png`, `morgath_v2_bolt.png` (strip 8 fase
  + konteks in-game + zoom flight/impact; generator:
