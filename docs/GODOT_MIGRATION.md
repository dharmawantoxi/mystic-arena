...
godot/
  ...
  scripts/
    ...
    autoload/AppShell.gd    # Port `main.py`: banner boot + crash_log.txt, kebijakan loop (60 Hz + catch-up 4),
                           # preset + adaptive quality, batas FPS, BGM/ambient sejak boot, siklus hidup app (FASE 27)
...
# Output: godot/assets/sounds/* (24 audio) -> AudioManager.gd
#         (16 .wav + 7 .ogg + 1 .mp3 — 8 berkas pygame bernama .wav padahal
#          isinya Ogg Vorbis/MP3; Godot memilih importer dari EKSTENSI jadi
#          salinannya diluruskan, pygame mengendus isi berkas sehingga di
#          sana namanya tidak masalah)
...
| **Fase 4f** | Blok `performance.py` + `fps_counter.py` `_system.py` (grid spasial, frustum culler, overlay FPS) | ✅ DONE (2026-09-11): `scripts/systems/SpatialGrid.gd` = paritas `_system.py:52-142` (`cell_size` 60, bbox prefilter → `dx²+dy² <= r²`, loop cy luar/cx dalam sehingga URUTAN hasil sama, `query_range` tanpa filter vs `query_enemies` buang sekutu+bangkai); `CombatSystem._spatial_grid` + `update_spatial_grid(_from_tree)` dijadwalkan `Main._process` tiap 2 frame (`GRID_REBUILD_EVERY`, paritas `animation_time % 2 == 0` `_core.py:2007-2013`), dan `query_enemies_in_range` = padanan `_system.py:170-185` (menara/nexus scan langsung appended terakhir, fallback scan grup saat grid basi). Konsumen: `Minion._find_target_smart` satu-satunya yang dialihkan (mage-chain/voly/splash/AoE tetap urutan grup yang sudah terkunci). `FrustumCuller.gd` (`:40-50`) dipakai `WorldPopups._draw`; `scenes/ui/FpsCounter.gd` (`:202-386`, riwayat 120, jendela 30, refresh 10 frame, `build_ops` = daftar perintah gambar murni) disaklarkan F8 di `Main._on_key` sebelum dispatch state (paritas `main_desktop_legacy.py:98-100`). TIDAK diport: `AdaptiveQuality` (`:400-448`, nol call site — ditegakkan tool oracle), pembekuan `death_anim` di luar layar (Godot mengurangi di `_process`), dan sampling FPS 2×/frame legacy (`:455-456` + `:461-462`). Oracle `tools/test_system_perf_parity.py` → fixture `godot/tests/fixtures/system_perf.json` → replay `tests/SystemPerfParityTest.tscn` (CI `godot-check.yml` 4u). Peta blok: `docs/SYSTEM_PY_COVERAGE.md` |
...
- `hero_items.py` → `godot/scripts/core/HeroItems.gd` port 1:1 (FASE 29): logika murni item yang belum punya pembaca Godot (kelas item `get_item_class`, halaman toko `build_shop_pages`, mekanik detail popup `build_item_mechanics`/`fmt_mech_value`, pesanan forge tertunda `pending_forge_items`/`deliver_pending_forge_items`, target toko `resolve_shop_target`, pengali level `hero_level_mult`). `ItemDB` mendelegasikan `item_class`/`item_class_label`/`item_mechanics`/`shop_pages` (badge kelas kartu toko tampil di ShopPanel). Dikunci seksi fixture `hero_items` + `HeroItemsParityTest`. **Ikon item (`get_icon` + `_ICON_CACHE`, `hero_items.py:1661-1699`) → `scripts/utils/ItemIcons.gd` + `ItemDB.item_icon()`**: 33 PNG `assets/items/` disalin `tools/convert_to_godot.py --assets` ke `godot/assets/items/` (ikut repo), dimuat dengan penjaga `ResourceLoader.exists()` lalu di-scale CPU ke ukuran tampilan (paritas `smoothscale`, cache per (item, ukuran) — hemat VRAM di Android), dan tanpa berkasnya mundur ke badge prosedural warna katalog — dipakai chip slot item `SkillBar` (30 px, ikon 26 px) serta baris toko `ShopPanel` tab ITEM. Dikunci `HeroItemsParityTest._test_icons` + `tools/test_godot_asset_pipeline.py`.
- `localization.py` → `godot/scripts/utils/Localization.gd` port 1:1 (FASE 30; aktif in-match FASE 39): tabel teks 24 kunci × 2 bahasa saat port awal, kini 106 kunci × 2 bahasa, bahasa aktif `static var`, `set_language`/`get_language`/`get_language_label`, dan `tr()` → `tr_text(key, Dictionary)` (nama `tr` dihindari karena `Object.tr()` native engine). Semantik fallback Python dipertahankan lewat `_py_format`/`_py_str` (kunci tak dikenal → kunci mentah, nilai placeholder hilang → template mentah, `3.0` → `"3.0"`). Pemakai: baris BAHASA di `MainMenu` PENGATURAN (+cycler), `GameManager.language`/`set_language`/`apply_language`/`language_changed`, `SaveManager.get_setting_str`/`set_setting_str`, boot sync `AppShell._apply_interface_language`, dan `ItemDB.item_mechanics_localized`. Sejak 2026-09-13 teksnya juga dipakai DI DALAM GAME — `HUD` (baris hint), `ShopPanel` (judul/tab/isi 4 tab), `SkillBar` (label + tooltip), `GameOverOverlay` (tombol + hint + catatan hasil) — semuanya menyambung `language_changed` (FASE 39; lihat `docs/GODOT_PARITY.md`). Dikunci `tools/test_godot_localization_parity.py` (oracle tanpa engine) + fixture `localization.json` + `LocalizationParityTest`. Rincian: `docs/LOCALIZATION_GODOTPP.md`.
