...
**Pembaruan pelaksanaan:** pengguna kemudian meminta implementasi langsung di repo karena keterbatasan waktu. Fondasi baru kini dibuat di [`godot_rebuild/`](../godot_rebuild/README.md), ditargetkan untuk Windows 11 dan Godot 4.7.2. Langkah manual di bawah tetap menjadi acuan, tetapi pengguna tidak perlu membuat ulang file yang sudah disediakan. Status checkpoint: menu/input/pause, minion, tower/projectile/nexus, serta prototipe level 1/normal dengan wave otomatis, gold dan build/sell Archer telah diimplementasikan; CI Godot 4.7.2 lulus 1.511 pemeriksaan native pada commit `ff5d2af`. Lawan prototipe masih builder terjadwal, bukan AI asli. Lihat [status implementasi dan handoff](STATUS_GODOT_REBUILD.md). Ini belum migrasi gameplay penuh dan game Python tidak diubah.
