...
**Pembaruan pelaksanaan:** pengguna kemudian meminta implementasi langsung di repo karena keterbatasan waktu. Fondasi baru kini dibuat di [`godot_rebuild/`](../godot_rebuild/README.md), ditargetkan untuk Windows 11 dan Godot 4.7.2. Langkah manual di bawah tetap menjadi acuan, tetapi pengguna tidak perlu membuat ulang file yang sudah disediakan. Status awal: menu/arena percobaan/input/pause telah diimplementasikan; pemeriksaan statis lulus, pengujian native engine masih menunggu. Ini belum migrasi gameplay penuh dan game Python tidak diubah.
