"""Optional in-game Sylara preview overlay.

Same idea as ``mobile/vex_review.py`` / ``mobile/grimjaw_review.py`` but for
the Sylara V1 pixel-art renderer: the renderer itself stays procedural and the
review sheets are deliberately opt-in, so normal arena frames never load or
draw a static PNG.  In review mode both the body animation sheet and the
Q/W/E/R skill sheet are shown in the right-side card area.  The card layout
engine is shared with ``mobile.kaizen_review`` so every overlay looks
identical.

Enable at launch with ``MYSTIC_SYLARA_REVIEW=1`` or toggle it with F7 on
...
_ASSET_PATH = _REVIEW_DIR / "sylara_v1_anim_sheet.png"
_SKILL_ASSET_PATH = _REVIEW_DIR / "sylara_v1_skill_sheet.png"


def _env_enabled() -> bool:
    return os.environ.get("MYSTIC_SYLARA_REVIEW", "0").strip().lower() in {
        "1", "true", "yes", "on"
    }
...
def toggle() -> bool:
    """Toggle the review cards (used by the desktop F7 shortcut)."""
...
def _load_sheet(path: Path):
    ...
    if not path.is_file():
        if key not in _warned_missing:
            print("[SYLARA REVIEW] asset tidak ditemukan: %s" % path)
    ...
    try:
        ...
        if key not in _warned_missing:
            print("[SYLARA REVIEW] gagal memuat asset: %s" % exc)
            _warned_missing.add(key)
        return None
    _sheets[key] = cached
    return cached
...
def draw(surface, panel_rect=None, font_getter=None) -> bool:
    """Draw both Sylara review sheets when the opt-in switch is enabled.
    ...
    def _draw(source_path, card_rect, image_rect, title, footer, fg):
        pygame.draw.rect(surface, (9, 18, 11), card_rect, border_radius=9)
        pygame.draw.rect(surface, (96, 158, 84), card_rect, 2,
                         border_radius=9)
        pygame.draw.line(
            surface,
            (196, 240, 150),
    ...
    for index, (path, title, footer) in enumerate((
        (_ASSET_PATH, "SYLARA V1 ANIMATION",
         "IDLE  \u2022  WALK  \u2022  ATTACK  \u2022  SWING"),
        (_SKILL_ASSET_PATH, "SYLARA V1 SKILLS",
         "Q FOCUS  \u2022  W WINDRUN  \u2022  E SHACKLE  \u2022  R POWERSHOT"),
    )):
        source = _load_sheet(path)
        if source is None:
            continue
