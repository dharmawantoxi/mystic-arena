"""Optional in-game Grimjaw preview overlay.

Same idea as ``mobile/vex_review.py`` but for the Grimjaw V1 pixel-art
...
``mobile/kaizen_review`` so all overlays always look identical.

Enable at launch with ``MYSTIC_GRIMJAW_REVIEW=1`` or toggle it with F11 on
...
_ASSET_PATH = _REVIEW_DIR / "grimjaw_v1_anim_sheet.png"
_SKILL_ASSET_PATH = _REVIEW_DIR / "grimjaw_v1_skill_sheet.png"


def _env_enabled() -> bool:
    return os.environ.get("MYSTIC_GRIMJAW_REVIEW", "0").strip().lower() in {
        "1", "true", "yes", "on"
    }
...
def toggle() -> bool:
    """Toggle the review cards (used by the desktop F11 shortcut)."""
...
def _load_sheet(path: Path):
    ...
    if not path.is_file():
        if key not in _warned_missing:
            print("[GRIMJAW REVIEW] asset tidak ditemukan: %s" % path)
    ...
    try:
        ...
        if key not in _warned_missing:
            print("[GRIMJAW REVIEW] gagal memuat asset: %s" % exc)
            _warned_missing.add(key)
        return None
    _sheets[key] = cached
    return cached
...
def draw(surface, panel_rect=None, font_getter=None) -> bool:
    """Draw both Grimjaw review sheets when the opt-in switch is enabled.
    ...
    # Same defensive local copy of the kaizen drawing routine as the Vex
    # overlay; only the accent colors are Grimjaw ember instead of void.
    def _draw(source_path, card_rect, image_rect, title, footer, fg):
        pygame.draw.rect(surface, (21, 12, 9), card_rect, border_radius=9)
        pygame.draw.rect(surface, (200, 110, 50), card_rect, 2,
                         border_radius=9)
        pygame.draw.line(
            surface,
            (255, 170, 80),
    ...
    for index, (path, title, footer) in enumerate((
        (_ASSET_PATH, "GRIMJAW V1 ANIMATION",
         "IDLE  \u2022  WALK  \u2022  ATTACK  \u2022  SPIN"),
        (_SKILL_ASSET_PATH, "GRIMJAW V1 SKILLS",
         "Q FURY  \u2022  W WARD  \u2022  E CRIT  \u2022  R OMNI"),
    )):
        source = _load_sheet(path)
        if source is None:
            continue
