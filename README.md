...
> **Godot — proyek baru dari nol:** buka [`godot_rebuild/project.godot`](godot_rebuild/project.godot) dengan Godot **4.7.2 standard**. [Panduan menjalankan di Windows dan status pengujian](godot_rebuild/README.md). Sudah ada fondasi, laboratorium minion/tower/nexus, serta **Pertandingan awal** dengan wave otomatis, gold dan build/sell Archer yang lulus tes native; **belum port gameplay penuh**. Tidak memakai hasil migrasi Godot lama. Bagian Python dan panduan lama di bawah tetap disimpan sebagai referensi.
