...
android.extra_manifest_application_arguments incorrectly when it calls
p4a through subprocess(list). To keep Android 12+ backup rules without
editing the GitHub workflow, the build uses this p4a hook instead:
after p4a renders AndroidManifest.xml, add
android:dataExtractionRules to the <application> element directly.

Cloud Save tidak lagi memakai Google Play Console, jadi hook ini TIDAK
menyisipkan meta-data Play Games (com.google.android.gms.games.APP_ID).
"""
