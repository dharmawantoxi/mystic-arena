...
#
# HANYA pakai library standar Python — tidak perlu pip install.
# Server ini menyimpan satu file JSON per pemain:
#     <data_dir>/<sha256(player_id)>.json
#
# Endpoint
# --------
...
#   PORT                      default 8080
#   MYSTIC_CLOUD_DATA_DIR     default ./cloud_data
#   MYSTIC_CLOUD_API_KEY      (opsional) kunci bersama; kalau kosong
#                             server menerima tanpa auth (hanya utk
#                             uji lokal — jangan dipakai di internet)
#
# Menjalankan:
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_PUT(self):
        ...
        print("[CLOUD] save %s -> %s (%d bytes)" % (
            pid[:40], filename_for(pid), len(body)))
...
def main():
    ...
    print("  data dir : %s" % os.path.abspath(DATA_DIR))
