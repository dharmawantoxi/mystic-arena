#!/usr/bin/env python3
"""Regression test VOKRAHN V3 (renderer + live combat-FX engine).

Mengunci kontrak yang dibangun di dokumen docs/VOKRAHN_V3_COMBAT_FX.md:

  * lapisan hidup heroes/vokrahn_fx.py punya semua API yang dipakai
    pipeline hero/boss (attach/owns/tick/draw_*_layer/notify_*);
  * palette karakter dipakai konsisten renderer <-> lapisan FX;
  * animation controller (_NS_vokrahn): state (12), prioritas, fase
    serangan, hit window, delta time;
  * busur: SWORD_ARC kontinu tanpa teleport, dan renderer <-> FX
    membaca SATU sumber geometri yang sama (sword_geometry);
  * dash (skill E): offset visual 80 px dari SATU fungsi
    (_NS_vokrahn._dash_offset, dikonversi ke basis waktu FX);
  ...
  * renderer bosses/level4.draw_vokrahn hidup bersama lapisan FX tanpa
  ...
  * boss AI (base_boss.py) memanggil lapisan FX vokrahn;
...
Jalankan:
  SDL_VIDEODRIVER=dummy SDL_AUDIODRIVER=dummy \
      python3 tools/test_vokrahn_v3_combat.py
...
from heroes import vokrahn_fx as F
...
def make_boss(x=100.0, y=100.0, skill=None, timer=0, target=True,
              target_dist=200.0):
    b = SimpleNamespace(
        boss_type="vokrahn", boss_class="mini", team="red",
...
def warm(surface, boss, x=350, y=350, frames=4):
    for _ in range(frames):
        L.draw_vokrahn(surface, boss, x, y)
        F.tick(1.0 / 60.0)
        time.sleep(0.001)
...
def test_module_api():
    required = [
        ...
        "ChaosBrandProjectile", "ProjectileSystem", "SkillFX",
        "VokrahnFXDirector",
        ...
        "VOKRAHN_PALETTE", "DEBUG_CHARACTER", "VOKRAHN_FX_ENABLED",
        "sword_arc", "attack_phase", "skill_phase",
        "sword_points", "sword_tip", "sword_angle", "launch_point",
        "dash_offset", "pose_of", "render_scale", "body_scale",
    ...
    for name in required:
        assert hasattr(F, name), "API hilang: vokrahn_fx.%s" % name
    ...
    for name in ("PALETTE", "SKILL_DUR", "SKILL_RADIUS", "ChaosBolt",
                 "ImpactBurst", "draw_vokrahn", "draw_boss",
                 "_update_attack_anim", "_manage_projectiles",
                 "_spawn_chaos_bolt", "_spawn_burst", "_target_position",
                 "_fx_scale" if hasattr(NS, "_fx_scale") else "GROUND_DY",
                 "_live_module", "live_fx_ready", "_draw_sword_swing_arc",
                 "_draw_swing_impact", "_draw_dash_trail_fx",
                 # v3
                 "DEBUG_CHARACTER", "ANIM_STATES", "ATTACK_PHASES",
                 "ATTACK_ACTIVE_WINDOW", "ATTACK_IMPACT_FRAME",
                 "MELEE_REACH", "_SWORD_HALF", "SWORD_ARC",
                 "attack_phase", "attack_phases_order",
                 "_sword_lift", "_sword_arc", "sword_geometry",
                 "_dash_offset", "_update_vok_anim", "_resolve_pose_vok",
                 "_draw_vok_body", "_draw_vok_body_raw",
                 "_draw_vokrahn_debug", "_last_rig", "draw_boss"):
        assert hasattr(NS, name), "API renderer hilang: _NS_vokrahn.%s" % name
    assert callable(L.draw_vokrahn)
...
def test_palette_contract():
    ...
    bands = (
        "arm_darkest", "arm_dark", "arm_mid", "arm_light", "arm_high",
        "arm_shine",
        "chaos_darkest", "chaos_dark", "chaos_mid", "chaos_bright",
        "chaos_hot", "chaos_glow",
        "fire_darkest", "fire_dark", "fire_mid", "fire_bright",
        "fire_hot", "fire_glow", "fire_white",
        "cape_darkest", "cape_dark", "cape_mid", "cape_light",
        ...
        "smoke_dark", "smoke_mid", "smoke_light", "smoke_glow",
        "ember", "ash", "dust",
        "outline", "shadow", "dark", "body", "mid", "light",
        "highlight", "weapon", "fx",
    )
    for k in bands:
        assert k in F.VOKRAHN_PALETTE, "palet FX kurang kunci %s" % k
    for k, v in F.VOKRAHN_PALETTE.items():
    ...
    for fx_key, rend_key in F._PALETTE_SYNC.items():
        assert fx_key in F.VOKRAHN_PALETTE, fx_key
...
def test_anim_controller():
    boss = make_boss()
    NS._update_vok_anim(boss, moving=False)
    assert boss._vok_state == "IDLE", boss._vok_state

    boss._vok_speed = 0.5
    NS._update_vok_anim(boss, moving=True)
    assert boss._vok_state == "WALK", boss._vok_state
    boss._vok_speed = 2.4
    NS._update_vok_anim(boss, moving=True)
    assert boss._vok_state == "RUN", boss._vok_state
    boss._vok_speed = 0.0

    boss.active_skill = "q"
    boss.active_skill_timer = 25
    NS._update_vok_anim(boss, moving=False)
    assert boss._vok_state == "SKILL", boss._vok_state
    boss.active_skill = "r"
    NS._update_vok_anim(boss, moving=False)
    assert boss._vok_state == "SPECIAL", boss._vok_state
    boss.active_skill = None
    boss.active_skill_timer = 0

    boss.hurt_flash_timer = 6
    NS._update_vok_anim(boss, moving=False)
    assert boss._vok_state == "HURT", boss._vok_state
    boss.hurt_flash_timer = 0

    boss.alive = False
    NS._update_vok_anim(boss, moving=False)
    assert boss._vok_state == "DEATH", boss._vok_state
    boss.alive = True
    ...
    boss.timer = 44
    NS._update_vok_anim(boss, moving=False)
    assert boss._vok_attack_active, "serangan tidak terdeteksi"
    phases = []
    for _ in range(46):
        NS._update_vok_anim(boss, moving=False)
        phases.append(boss._vok_attack_phase)
        assert 0.0 < boss._vok_dt <= 0.05
...
def test_always_melee():
    """Vokrahn SELALU melee (greatsword) — ayunan, bukan bidikan."""
    boss = make_boss()
    boss.timer = 44
    NS._update_vok_anim(boss)
    seen = set()
    for _ in range(40):
        NS._update_vok_anim(boss)
        seen.add(boss._vok_state)
    assert "SWING" in seen, "ayunan besar tidak pernah terjadi (%s)" % seen
    assert "CAST" not in seen, \
        "vokrahn tidak menembak: state CAST muncul (%s)" % seen
...
def test_sword_arc_continuity():
    ...
    max_step = 0.0
    for i in range(201):
        phi, lift = NS._sword_arc(i / 200.0)
    ...
    assert F.SWORD_ARC == NS.SWORD_ARC, \
        "tabel SWORD_ARC FX menyimpang dari renderer"


def test_sword_geometry_single_source():
    # panjang bilah konstan di semua pose (tidak melar/menyusut)
    L_half = NS._SWORD_HALF
    for action in ("idle", "walk", "swing", "attack", "cast", "skill",
                   "hurt", "death", "q_cast", "w_cast", "e_cast", "r_cast"):
        for p in (0.0, 0.25, 0.5, 0.75, 1.0):
            grip, tip, lo, phi = NS.sword_geometry(1, action, 0.0, p)
    ...
    # mirror facing
    g1, t1, _l1, _t = NS.sword_geometry(1, "swing", 0.0, 0.4)
    g2, t2, _l2, _t2 = NS.sword_geometry(-1, "swing", 0.0, 0.4)
    ...
    # NOTE: set pose SETELAH draw (draw mem-rewrite _vok_pose_action).
    boss = make_boss()
    surf = pygame.Surface((400, 400), pygame.SRCALPHA)
    L.draw_vokrahn(surf, boss, 200.0, 200.0)
    boss._vok_pose_action = "swing"
    boss._vok_attack_progress = 0.4
    boss._vok_phase = 0.0
    grip, tip, lo = F.sword_points(boss, 200.0, 200.0)
    g, t, l4, _t = NS.sword_geometry(1, "swing", 0.0, 0.4)
    ...
    ang = F.sword_angle(boss, 200.0, 200.0)
    assert abs(ang - math.atan2(tip[1] - lo[1], tip[0] - lo[0])) < 1e-9
    F.reset_all()


def test_dash_offset_bridge():
    ...
    for p in (0.0, 0.15, 0.302, 0.4, 0.55, 0.605, 0.7, 0.85, 1.0):
        fx = F.dash_offset(None, p * span)
        rn = NS._dash_offset(p)
        assert abs(fx - rn) < 1e-9, (p, fx, rn)
    # puncak 80 px di tengah dash, 0 saat engine selesai
    peak = max(F.dash_offset(None, i / 200.0 * span) for i in range(201))
    assert abs(peak - 80.0) < 1e-6, peak
    assert abs(NS._dash_offset(1.0)) < 1e-9
    # setelah engine selesai, FX tetap diam di posisi awal (tidak
    # "kembali" di ekor AFTER: badan sudah di posisi logisnya)
    assert F.dash_offset(None, 1.0) == 0.0
...
def test_swing_trail_records_history():
    ...
    boss._vok_pose_action = "swing"
    boss._vok_attack_progress = 0.0
    for i in range(1, 30):
        boss._vok_attack_progress = i / 30.0
...
def test_bolt_is_not_a_plain_circle():
    """Bolt harus punya bentuk terarah + rotasi, bukan bulatan polos."""
    F.reset_all()
    d = F.VokrahnFXDirector()
    a = d.projectiles.spawn_bolt(100.0, 100.0, 400.0, 100.0)
    b = d.projectiles.spawn_bolt(100.0, 100.0, 100.0, 400.0)
    for _ in range(10):
...
def test_brand_projectile():
    F.reset_all()
    d = F.VokrahnFXDirector()
    br = d.projectiles.spawn_brand(100.0, 100.0, 100.0 + 260.0, 100.0)
    assert br.active and br.kind == "brand"
    for _ in range(400):
        d.update(1.0 / 60.0)
        if not br.active:
            break
    assert not br.active, "chaos brand tidak pernah selesai"
    assert len(br.trail) > 2, "brand tidak punya trail"
    F.reset_all()
...
def test_projectile_cap():
    ...
    for _ in range(F.MAX_PROJECTILES + 20):
        d.projectiles.spawn_bolt(0.0, 0.0, 400.0, 0.0)
...
def test_skill_radius_matches_gameplay():
    ...
    assert F.WORLD_RADIUS == {"q": 250.0, "w": 220.0, "e": 150.0,
                              "r": 220.0}
    assert F.SKILL_DUR == {"q": 50, "w": 60, "e": 50, "r": 80}
    ...
    zone = src[src.index("def _cast_vokrahn_q"):
               src.index("def _smart_ai_nyxara")]
    for dur in (50, 60, 80):
        assert "self.active_skill_timer = %d" % dur in zone, dur
    assert zone.count("self.active_skill_timer = 50") == 2, \
        "Q & E sama-sama 50 frame"
    assert "<= 220" in zone, "radius damage AI W/R tidak lagi 220"
...
def test_aoe_telegraph_is_a_true_circle():
    """W & R (220) HARUS punya cincin lingkaran di radius dunia.
    ...
    for skill in ("w", "r"):
        ...
        n = _hits_ring(surf, 220.0, cx, cy + 10)
        assert n > 110, "%s: cincin AoE 220 dunia meleset (%d/180)" % (
            skill.upper(), n)
    F.reset_all()
...
def test_renderer_with_live_fx():
    ...
    assert getattr(boss, "_vok_suppress_canvas_fx", False), \
...
def test_renderer_all_states():
    ...
    atk = make_boss()
    atk._vok_attack_active = True
    atk._vok_attack_frame = 14
    atk._vok_prev_timer = 44
    atk.timer = 43
...
def test_canvas_fallback_when_nobody_draws_live_layer():
    ...
    surf = pygame.Surface((700, 700), pygame.SRCALPHA)
    L.draw_vokrahn(surf, b, 350, 350)
    assert not getattr(b, "_vok_suppress_canvas_fx", False), \
...
def test_hero_lane_pipeline():
    ...
    F.reset_all()
    h = SimpleNamespace(
        hero_type="vokrahn", boss_type="vokrahn", boss_class="mini",
    ...
    for _ in range(8):
        H.render_hero("vokrahn", surf, h, 280, 280)
        F.tick(1.0 / 60.0)
        time.sleep(0.001)
...
def test_additive_blit_respects_alpha():
    ...
    R = 120
    glow = F.glow_surface(R, F.P["chaos_bright"], power=1.0)
...
def test_dash_afterimages_bounded():
    """Dash E meninggalkan afterimage (dari rig tersimpan) yang
    terbatas jumlahnya dan meluruh sendiri."""
    F.reset_all()
    b = make_boss(skill="e", timer=25)
    surf = pygame.Surface((700, 700), pygame.SRCALPHA)
    ...
    assert peak >= 1, "dash tidak meninggalkan afterimage"
    for _ in range(300):
        F.tick(1.0 / 60.0)
...
def test_cache_bounded():
    ...
    surf = pygame.Surface((700, 700), pygame.SRCALPHA)
    for _ in range(90):
        L.draw_vokrahn(surf, boss, 350, 350)
        F.tick(1.0 / 60.0)
...
def test_registry():
    import re
    import heroes as H
    assert "vokrahn" in H._LIVE_FX_HEROES
    assert H._LIVE_FX_PATHS.get("vokrahn") == "heroes.vokrahn_fx"
    ...
    src = open(os.path.join(ROOT, "bosses", "base_boss.py"),
               "r", encoding="utf-8").read()
    m = re.search(r'BOSS_LABEL_TOP\s*=\s*\{[^}]*"vokrahn":\s*(\d+)', src,
                  re.S)
    assert m, "BOSS_LABEL_TOP tidak punya entri vokrahn"
    assert int(m.group(1)) >= 80, \
        "BOSS_LABEL_TOP vokrahn terlalu rendah -> nama menutupi kepala"
...
def test_procedural_only():
    ...
    """
    for rel in (("heroes", "vokrahn_fx.py"), ("bosses", "level4.py")):
...
def test_frame_budget():
    ...
    boss = make_boss(skill="r", timer=40)
    surf = pygame.Surface((900, 700), pygame.SRCALPHA)
    L.draw_vokrahn(surf, boss, 450, 380)          # warm cache
    F.tick(1.0 / 60.0)
    t0 = time.perf_counter()
    for _ in range(60):
        surf.fill((0, 0, 0, 0))
        L.draw_vokrahn(surf, boss, 450, 380)
        F.tick(1.0 / 60.0)
...
TESTS = [
    ...
    test_bolt_lifecycle,
    test_bolt_is_not_a_plain_circle,
    test_brand_projectile,
    test_projectile_cap,
...
if __name__ == "__main__":
    ...
    if failed:
        ...
        sys.exit(1)
    print("SEMUA %d TEST VOKRAHN V3 LULUS" % len(TESTS))
