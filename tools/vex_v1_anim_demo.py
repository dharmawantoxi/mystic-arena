#!/usr/bin/env python3
# ============================================================
# Demo: sprite-sheet animasi Vex V1 (walk, attack, idle)
# ------------------------------------------------------------
# Render frame-frame animasi rig pixel-art V1 Vex, berdampingan,
# untuk melihat & membandingkan kualitas animasi - pola yang sama
# dengan tools/kaizen_anim_demo.py.
#
# Jalankan:
#   SDL_VIDEODRIVER=dummy SDL_AUDIODRIVER=dummy \
#     python3 tools/vex_v1_anim_demo.py
# ============================================================
import os
import sys
...
# lapisan hidup 60fps (heroes/vex_fx.py) dimatikan.
try:
    from heroes import vex_fx as _vxfx
    _vxfx.VEX_FX_ENABLED = False
    vx._LIVE_MOD = False
except Exception:
    pass
...
def render_frame(action, phase, progress=0.0, facing=1, size=150):
    """Render satu frame body rig (tanpa efek latar)."""
    s = pygame.Surface((size, size), pygame.SRCALPHA)
    vx._draw_vex_elite(s, size // 2, size // 2, facing, phase, action,
                       progress, True)
    return s


def render_cycle(action, n_frames, size=150, progress=0.0):
    frames = []
    params = []
...
def _compose(title, frames, params, size=150, pad=6):
    n = len(frames)
    w = n * (size + pad) + pad
    h = size + pad * 2 + 34
...
def main():
    ...
    out_path = os.path.join(out, "vex_v1_anim_sheet.png")
    asset_path = os.path.join(asset_dir, "vex_v1_anim_sheet.png")
