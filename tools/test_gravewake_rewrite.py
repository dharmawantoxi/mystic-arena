#!/usr/bin/env python3
"""Regression test permanen untuk sistem tempur GRAVEWAKE v3 (rewrite).

Gravewake "The Tidehunter" adalah MINI BOSS level 6 yang digambar lewat
``bosses/level6.py::_NS_gravewake``, dengan lapisan FX hidup di
``heroes/gravewake_fx.py`` dan bus game-feel bersama ``heroes/combat_feel.py``
(sama persis dengan arsitektur Thalgryn v3 / Kunkka v3 / Syrentha v3). Karena
itu kontraknya dipecah:

  * ``bosses/level6.py::_NS_gravewake`` = badan kraken (rok tentakel +
    torso otot + sabuk kulit + lengan + jangkar berkarat + kepala runcing),
    rig, pose, controller animasi, telegraph tanah, Anchor Wave & efek
    canvas (fallback).
  * ``heroes/gravewake_fx.py``         = lapisan hidup 1:1 di luar sprite
    cache: pita tebasan jangkar, partikel air, Anchor Wave (proyektil),
...
Uji ini mengunci kontrak yang gampang rusak:
  ...
  3. Controller animasi: fase, urutan, delta-time, jendela hit, prioritas
     state, dan semua nama atribut lama (``_gw_*``).
  4. Tebasan jangkar berbasis busur (bukan lerp linear).
  5. Particle system berbatas (cap dihormati, meluruh).
  6. Lifecycle Anchor Wave (spawn -> travel -> hit -> impact -> mati).
...
Jalankan:  python3 -m pytest tools/test_gravewake_rewrite.py -q
"""
import os
import sys
import time
...
from heroes import gravewake_fx as F                       # noqa: E402
from bosses.level6 import _NS_gravewake as G               # noqa: E402

DT = 1.0 / 60.0
COOLDOWN = 46                     # attack_cooldown gravewake di boss_data.py
...
def fresh_hero(x=0.0, y=0.0, cooldown=COOLDOWN, scale=None):
    """Unit Gravewake minim. ``scale=None`` = jalur BOSS (tanpa _render_scale)."""
    h = _ProbeEntity("gravewake", x, y)
    h.boss_type = "gravewake"
    h.alive = True
    h.pulse = 1.2
...
def run_live(hero, frames, dt=DT, attack=False, skill=None, skill_dur=40,
    ...
    for i in range(frames):
        hero.pulse += 0.3
        if attack:
            hero._gw_attack_active = True
            hero._gw_attack_manual = True
            hero._gw_attack_progress = (i % COOLDOWN) / float(COOLDOWN)
        if skill is not None:
            hero.active_skill = skill
            hero.active_skill_timer = max(0, skill_dur - i)
            if hero.active_skill_timer == 0:
                hero.active_skill = None
        G._update_gravewake_attack_anim(hero)
        F.tick(dt)
        surf.fill((0, 0, 0, 0))
...
def test_tidak_memuat_asset_eksternal():
    for rel in (os.path.join("heroes", "gravewake_fx.py"),):
    ...
    # sumber _NS_gravewake juga tidak boleh memuat bitmap apa pun
...
def test_api_publik_renderer_tetap_ada():
    ...
    assert callable(G._update_gravewake_attack_anim)
    assert callable(G._resolve_anim_state)
    assert callable(G._resolve_pose)
    assert callable(G._tip_local) and callable(G._front_grip_local)


def test_palet_lengkap_dan_sinkron_dengan_renderer():
    need = ["skin_darkest", "skin_dark", "skin_mid", "skin_light",
            "skin_high", "skin_shine", "belly_dark", "belly_mid",
            "belly_light", "bone_darkest", "bone_dark", "bone_mid",
            "bone_light", "bone_shine", "leather_darkest",
            "leather_dark", "leather_mid", "leather_light",
            "anchor_darkest", "anchor_dark", "anchor_mid",
            "anchor_light", "anchor_shine", "anchor_rust",
            "water_darkest", "water_dark", "water_mid", "water_light",
            "water_bright", "water_hot", "water_white", "gold_darkest",
            "gold_dark", "gold_mid", "gold_light", "gold_shine",
            "teeth_dark", "teeth_mid", "teeth_light", "eye_dark",
            "eye_mid", "eye_bright", "eye_hot"]
    for k in need:
    ...
    assert F.P["anchor_mid"] == tuple(int(c) for c in G.PALETTE["anchor_mid"])
...
def test_progress_monoton_dan_habis_tepat():
    h = fresh_hero()
    h._gw_attack_active = False
    h._gw_attack_frame = 0
    h._gw_prev_timer = 0
    prev = 0.0
    for t in range(COOLDOWN, -1, -1):
        h.timer = t
        G._update_gravewake_attack_anim(h)
        p = float(h._gw_attack_progress)
        assert p >= prev - 1e-9, "progress mundur di timer=%d" % t
        prev = p
    assert abs(float(h._gw_attack_progress) - 1.0) < 1e-6


def test_jendela_hit_aktif_di_tengah_ayunan():
    h = fresh_hero()
    h._gw_attack_active = True
    h._gw_attack_manual = True
    h._gw_attack_progress = 0.40
    G._update_gravewake_attack_anim(h)
    assert h._gw_attack_phase in ("SWING", "IMPACT")
    lo, hi = G.ATTACK_ACTIVE_WINDOW
    assert lo <= 0.40 < hi
    assert h._gw_hit_active
...
def test_hurt_dari_flash_boss():
    ...
    G._update_gravewake_attack_anim(h)
    assert int(getattr(h, "_gw_hurt_frames", 0)) > 0


# ══════════════════════════════════════════════════════════════════════════
# 3. JANGKAR — busur, bukan lerp
# ══════════════════════════════════════════════════════════════════════════
def test_ujung_jangkar_mengikuti_busur_bukan_lerp():
    tips = []
    ...
    assert max(angs) - min(angs) > 0.2, "ujung jangkar bergerak lurus"


def test_kurva_serangan_monoton_dan_akselerasi_tengah():
...
def test_trail_dibangun_dari_histori_dan_meluruh():
    ...
    for i in range(8):
        h._gw_attack_active = True
        h._gw_attack_phase = "SWING" if i % 2 == 0 else "IMPACT"
        F.tick(DT)
...
def test_cast_q_meluncurkan_anchor_wave():
    h = fresh_hero()
...
def test_skill_durasi_sama_dengan_engine():
    # boss AI (bosses/base_boss.py _gravewake_q/w/e/r) dan skill hero
...
def test_skill_impact_tanpa_target_tidak_error():
    h = fresh_hero()
    F.notify_skill_impact(h, 10.0, 10.0, 200, "q")
    F.notify_skill_impact(h, 10.0, 10.0, 120, "w")
    F.notify_skill_impact(h, 10.0, 10.0, 150, "e")
    F.notify_skill_impact(h, 10.0, 10.0, 200, "r")
    d = F.director_for(h)
    assert len(d.skills) >= 1
...
def test_shake_meluruh_ke_nol():
    ...
    F.notify_skill_impact(h, 0.0, 0.0, 200, "r")
...
def test_impact_tanpa_target_tidak_error():
    h = fresh_hero()
    F.notify_projectile_impact(h, 30.0, 10.0, 0.0, 60, False, "wave")
    d = F.director_for(h)
...
def test_jalur_boss_mengambil_alih_effect():
    ...
    # jalur boss memuat lapisan hidup -> canvas anchor wave tidak di-spawn
    h.active_skill = "q"
    h.active_skill_timer = 28           # cast_progress ~ 0.38 (jendela spawn)
    h._gw_live_owned = True
    G.draw_gravewake(surf, h, 120, 150)
    assert len(getattr(h, "_gw_effects", [])) == 0
...
def test_fallback_canvas_saat_modul_fx_tidak_ada():
    ...
    try:
        ...
        h._gw_live_owned = False
        h._gw_swing_spawned = False
        G.draw_gravewake(surf, h, 120, 150)
        # canvas fallback spawn anchor wave saat jendela spawn (0.35-0.45)
        assert len(getattr(h, "_gw_effects", [])) >= 1
    finally:
        G._LIVE_MOD = old


def test_lane_hero_terdaftar_di_live_fx():
    assert "gravewake" in heroes._LIVE_FX_HEROES
    assert heroes._LIVE_FX_PATHS.get("gravewake") == "heroes.gravewake_fx"


def test_render_hero_menjalankan_lapisan_hidup():
    h = fresh_hero(scale=0.6)
    surf = pygame.Surface((320, 240), pygame.SRCALPHA)
    heroes._live_fx_pre("gravewake", surf, h, h.x, h.y)
    heroes._live_fx_post("gravewake", surf, h, h.x, h.y)
    # tidak boleh error dan director terpasang
    assert getattr(h, "_gw_live_fx", False)
...
def test_lapisan_hidup_cepat():
    ...
    for i in range(n):
        h._gw_attack_active = True
        h._gw_attack_progress = (i % COOLDOWN) / float(COOLDOWN)
        h._gw_attack_phase = PHASE_ORDER[min(5, i // 6)]
        if i % 10 == 0:
...
def test_draw_gravewake_lane_boss_dibawah_anggaran():
    h = fresh_hero()
    ...
    assert ms < 8.0, "draw_gravewake(live) %.2f ms/frame" % ms
