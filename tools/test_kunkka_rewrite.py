#!/usr/bin/env python3
"""Regression test permanen untuk sistem tempur KUNKKA v3 (rewrite).

Kunkka "The Admiral of the Fleet" adalah TRUE BOSS level 6 yang digambar
lewat ``bosses/level6.py::_NS_kunkka``, dengan lapisan FX hidup di
``heroes/kunkka_fx.py`` dan bus game-feel bersama ``heroes/combat_feel.py``
(sama persis dengan arsitektur Gornak v3 / Thalgryn v3). Karena itu
kontraknya dipecah:

  * ``bosses/level6.py::_NS_kunkka`` = badan admiral (coat navy + trim
    emas + cutlass), rig, pose, controller animasi, telegraph tanah,
    proyektil/efek canvas (fallback).
  * ``heroes/kunkka_fx.py``          = lapisan hidup 1:1 di luar sprite
    cache: pita tebasan cutlass, partikel air, Tide Wave, impact,
    hit-stop, shake, overlay debug.
...
Uji ini mengunci kontrak yang gampang rusak:
  ...
  3. Controller animasi: fase, urutan, delta-time, jendela hit, prioritas
     state, dan semua nama atribut lama (``_kk_*``).
  4. Ayunan cutlass berbasis busur (bukan lerp linear).
  5. Particle system berbatas (cap dihormati, meluruh).
  6. Lifecycle Tide Wave (spawn -> travel -> hit -> impact -> mati).
...
Jalankan:  python3 -m pytest tools/test_kunkka_rewrite.py -q
"""
import os
import sys
import time
...
from heroes import kunkka_fx as F                          # noqa: E402
from bosses.level6 import _NS_kunkka as G                  # noqa: E402

DT = 1.0 / 60.0
COOLDOWN = 42                     # attack_cooldown kunkka di boss_data.py
...
def fresh_hero(x=0.0, y=0.0, cooldown=COOLDOWN, scale=None):
    """Unit Kunkka minim. ``scale=None`` = jalur BOSS (tanpa _render_scale)."""
    h = _ProbeEntity("kunkka", x, y)
    h.boss_type = "kunkka"
    h.alive = True
    h.pulse = 1.2
...
def run_live(hero, frames, dt=DT, attack=False, skill=None, skill_dur=40,
    ...
    for i in range(frames):
        hero.pulse += 0.3
        if attack:
            hero._kk_attack_active = True
            hero._kk_attack_manual = True
            hero._kk_attack_progress = (i % COOLDOWN) / float(COOLDOWN)
        if skill is not None:
            hero.active_skill = skill
            hero.active_skill_timer = max(0, skill_dur - i)
            if hero.active_skill_timer == 0:
                hero.active_skill = None
        G._update_kunkka_attack_anim(hero)
        F.tick(dt)
        surf.fill((0, 0, 0, 0))
...
def test_tidak_memuat_asset_eksternal():
    for rel in (os.path.join("heroes", "kunkka_fx.py"),):
    ...
    # sumber _NS_kunkka juga tidak boleh memuat bitmap apa pun
...
def test_api_publik_renderer_tetap_ada():
    ...
    assert callable(G._update_kunkka_attack_anim)
    assert callable(G._resolve_anim_state)


def test_palet_lengkap_dan_sinkron_dengan_renderer():
    need = ["coat_darkest", "coat_mid", "coat_light", "gold_dark",
            "gold_mid", "gold_light", "water_darkest", "water_dark",
            "water_mid", "water_light", "water_bright", "blade_darkest",
            "blade_dark", "blade_mid", "blade_light", "ghost_dark",
            "ghost_mid", "ghost_light", "red_x", "red_hot"]
    for k in need:
    ...
    assert F.P["coat_mid"] == tuple(int(c) for c in G.PALETTE["coat_mid"])
...
def test_progress_monoton_dan_habis_tepat():
    h = fresh_hero()
    h._kk_attack_active = False
    h._kk_attack_frame = 0
    h._kk_previous_timer = 0
    prev = 0.0
    for t in range(COOLDOWN, -1, -1):
        h.timer = t
        G._update_kunkka_attack_anim(h)
        p = float(h._kk_attack_progress)
        assert p >= prev - 1e-9, "progress mundur di timer=%d" % t
        prev = p
    assert abs(float(h._kk_attack_progress) - 1.0) < 1e-6


def test_jendela_hit_aktif_di_tengah_ayunan():
    h = fresh_hero()
    h._kk_attack_active = True
    h._kk_attack_frame = int(0.4 * (COOLDOWN - 1))
    h._kk_attack_progress = 0.4
    h._kk_attack_phase = G.attack_phase(0.4)
    G._update_kunkka_attack_anim(h)
    assert h._kk_attack_phase in ("SWING", "IMPACT")
    lo, hi = G.ATTACK_ACTIVE_WINDOW
    assert lo <= 0.4 < hi
    assert h._kk_hit_active
...
def test_hurt_dari_flash_boss():
    ...
    G._update_kunkka_attack_anim(h)
    assert int(getattr(h, "_kk_hurt_frames", 0)) > 0


# ══════════════════════════════════════════════════════════════════════════
# 3. CUTLASS — busur, bukan lerp
# ══════════════════════════════════════════════════════════════════════════
def test_ujung_bilah_mengikuti_busur_bukan_lerp():
    ...
    assert max(angs) - min(angs) > 0.2, "ujung bilah bergerak lurus"
...
def test_trail_dibangun_dari_histori_dan_meluruh():
    ...
    for i in range(8):
        h._kk_attack_active = True
        h._kk_attack_phase = "SWING" if i % 2 == 0 else "IMPACT"
        F.tick(DT)
        F.draw_live_layer(pygame.Surface((360, 260), pygame.SRCALPHA),
                          h, h.x, h.y)
    d = F.director_for(h)
    assert len(d.trail.points) <= d.trail.samples
...
def test_cast_q_meluncurkan_tide_wave():
    h = fresh_hero()
...
def test_skill_durasi_sama_dengan_engine():
    # boss AI (bosses/base_boss.py _cast_q/w/e/r kunkka) dan skill hero
...
def test_skill_impact_tanpa_target_tidak_error():
    ...
    F.notify_skill_impact(h, 10.0, 10.0, 300, "e")
...
def test_impact_tanpa_target_tidak_error():
    h = fresh_hero()
    F.notify_projectile_impact(h, 30.0, 10.0, 0.0, 60, False, "tide")
    d = F.director_for(h)
...
def test_jalur_boss_mengambil_alih_effect():
    ...
    # jalur boss memuat lapisan hidup -> canvas tide wave tidak di-spawn
    h.active_skill = "q"
    h.active_skill_timer = 28           # cast_progress ~ 0.38 (jendela spawn)
    h._kk_live_owned = True
    G.draw_kunkka(surf, h, 120, 150)
    assert len(getattr(h, "_kk_effects", [])) == 0
...
def test_fallback_canvas_saat_modul_fx_tidak_ada():
    ...
    try:
        ...
        h._kk_live_owned = False
        h._kk_wave_spawned = False
        G.draw_kunkka(surf, h, 120, 150)
        # canvas fallback spawn tide wave saat jendela spawn (0.35-0.42)
        assert len(getattr(h, "_kk_effects", [])) >= 1
    finally:
        G._LIVE_MOD = old


def test_lane_hero_terdaftar_di_live_fx():
    assert "kunkka" in heroes._LIVE_FX_HEROES
    assert heroes._LIVE_FX_PATHS.get("kunkka") == "heroes.kunkka_fx"


def test_render_hero_menjalankan_lapisan_hidup():
    h = fresh_hero(scale=0.6)
    surf = pygame.Surface((320, 240), pygame.SRCALPHA)
    heroes._live_fx_pre("kunkka", surf, h, h.x, h.y)
    heroes._live_fx_post("kunkka", surf, h, h.x, h.y)
    # tidak boleh error dan director terpasang
    assert getattr(h, "_kk_live_fx", False)
...
def test_lapisan_hidup_cepat():
    ...
    for i in range(n):
        h._kk_attack_active = True
        h._kk_attack_progress = (i % COOLDOWN) / float(COOLDOWN)
        h._kk_attack_phase = PHASE_ORDER[min(5, i // 6)]
        if i % 10 == 0:
...
def test_draw_kunkka_lane_boss_dibawah_anggaran():
    h = fresh_hero()
    ...
    assert ms < 8.0, "draw_kunkka(live) %.2f ms/frame" % ms
