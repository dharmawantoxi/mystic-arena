#!/usr/bin/env python3
"""Regression test permanen untuk sistem tempur THALGRYN v3 (rewrite).

Thalgryn "The Shape of Water" adalah mini boss level 6 yang digambar lewat
``bosses/level6.py::_NS_thalgryn``, dengan lapisan FX hidup di
``heroes/thalgryn_fx.py`` dan bus game-feel bersama ``heroes/combat_feel.py``
(sama persis dengan arsitektur Gornak v3). Karena itu kontraknya dipecah:

  * ``bosses/level6.py::_NS_thalgryn`` = badan air, rig, pose, controller
    animasi, telegraph tanah, proyektil/efek canvas (fallback).
  * ``heroes/thalgryn_fx.py``          = lapisan hidup 1:1 di luar sprite
    cache: pita bilah air, partikel, Water Spear, impact, hit-stop, shake,
    overlay debug.
...
Uji ini mengunci kontrak yang gampang rusak:
  ...
  3. Controller animasi: fase, urutan, delta-time, jendela hit, prioritas
     state, dan semua nama atribut lama (``_th_*``).
  4. Ayunan bilah air berbasis busur (bukan lerp linear).
  5. Particle system berbatas (cap dihormati, meluruh).
  6. Lifecycle Water Spear (spawn -> travel -> hit -> impact -> mati).
  ...
  9. Supresi ganda: lapisan hidup mengambil alih ayunan/lemparan -> canvas
     fallback tidak menggambar dua kali; fallback tetap jalan kalau modul
     FX tidak dimuat.
 10. Overlay debug + registrasi lane hero + performansi.

Jalankan:  python3 -m pytest tools/test_thalgryn_rewrite.py -q
"""
import os
import sys
import time
...
DT = 1.0 / 60.0
COOLDOWN = 44                     # attack_cooldown thalgryn di boss_data.py
...
def fresh_hero(x=0.0, y=0.0, cooldown=COOLDOWN, scale=None):
    """Unit Thalgryn minim. ``scale=None`` = jalur BOSS (tanpa _render_scale)."""
...
def test_tidak_memuat_asset_eksternal():
    for rel in (os.path.join("heroes", "thalgryn_fx.py"),):
    ...
    # sumber _NS_thalgryn juga tidak boleh memuat bitmap apa pun
...
def test_api_publik_renderer_tetap_ada():
    assert callable(G.draw_thalgryn)
...
def test_palet_lengkap_dan_sinkron_dengan_renderer():
    need = ["water_darkest", "water_dark", "water_mid", "water_light",
            "water_high", "water_shine", "blade_dark", "blade_mid",
            "blade_light", "blade_shine", "eye_mid", "eye_bright",
            "gold_mid", "str_red", "agi_green", "int_blue"]
    for k in need:
...
def test_progress_monoton_dan_habis_tepat():
    h = fresh_hero()
    h._th_attack_active = False
    h._th_attack_frame = 0
    h._th_previous_timer = 0
    prev = 0.0
    for t in range(COOLDOWN, -1, -1):
        h.timer = t
        G._update_thalgryn_attack_anim(h)
        p = float(h._th_attack_progress)
        assert p >= prev - 1e-9, "progress mundur di timer=%d" % t
        prev = p
    assert abs(float(h._th_attack_progress) - 1.0) < 1e-6


def test_jendela_hit_aktif_di_tengah_ayunan():
    h = fresh_hero()
    h._th_attack_active = True
    h._th_attack_frame = int(0.4 * (COOLDOWN - 1))
    h._th_attack_progress = 0.4
    h._th_attack_phase = G.attack_phase(0.4)
    G._update_thalgryn_attack_anim(h)
    assert h._th_attack_phase in ("SWING", "IMPACT")
...
def test_hurt_dari_flash_boss():
    ...
    G._update_thalgryn_attack_anim(h)
    assert int(getattr(h, "_th_hurt_frames", 0)) > 0


# ══════════════════════════════════════════════════════════════════════════
# 3. BILAH AIR — busur, bukan lerp
# ══════════════════════════════════════════════════════════════════════════
...
def test_trail_dibangun_dari_histori_dan_meluruh():
    ...
    for i in range(8):
        h._th_attack_active = True
        h._th_attack_phase = "SWING" if i % 2 == 0 else "IMPACT"
        F.tick(DT)
...
def test_skill_durasi_sama_dengan_engine():
    # boss AI (bosses/base_boss.py _thalgryn_q/w/e/r) dan skill hero
...
def test_skill_impact_tanpa_target_tidak_error():
    h = fresh_hero()
    F.notify_skill_impact(h, 10.0, 10.0, 120, "q")
...
def test_impact_tanpa_target_tidak_error():
    h = fresh_hero()
    F.notify_projectile_impact(h, 30.0, 10.0, 0.0, 60, False, "spear")
    d = F.director_for(h)
...
def test_jalur_boss_mengambil_alih_effect():
    ...
    # jalur boss memuat lapisan hidup -> canvas proyektil tidak di-spawn
    h.timer = COOLDOWN
    h._th_attack_active = True
    h._th_attack_progress = 0.51
    h._th_attack_phase = "IMPACT"
    h._th_live_owned = True
    G.draw_thalgryn(surf, h, 120, 150)
    assert len(getattr(h, "_th_projectiles", [])) == 0
...
def test_fallback_canvas_saat_modul_fx_tidak_ada():
    ...
    try:
        surf = pygame.Surface((320, 240), pygame.SRCALPHA)
        # mode manual (timer=0): controller menghormati progress yang
        # diberikan oleh pemanggil (pola tools/_audit_*, paritas Gornak).
        h.timer = 0
        h._th_attack_active = True
        h._th_attack_manual = True
        h._th_attack_progress = 0.51
        h._th_live_owned = False
        G.draw_thalgryn(surf, h, 120, 150)
        # canvas fallback spawn spear saat jendela impact (raw ~0.51)
        assert len(getattr(h, "_th_projectiles", [])) >= 1
    finally:
        G._LIVE_MOD = old


def test_lane_hero_terdaftar_di_live_fx():
    assert "thalgryn" in heroes._LIVE_FX_HEROES
    assert heroes._LIVE_FX_PATHS.get("thalgryn") == "heroes.thalgryn_fx"


def test_render_hero_menjalankan_lapisan_hidup():
    h = fresh_hero(scale=0.6)
    surf = pygame.Surface((320, 240), pygame.SRCALPHA)
    heroes._live_fx_pre("thalgryn", surf, h, h.x, h.y)
    heroes._live_fx_post("thalgryn", surf, h, h.x, h.y)
    # tidak boleh error dan director terpasang
    assert getattr(h, "_th_live_fx", False)
...
def test_lapisan_hidup_cepat():
    ...
    for i in range(n):
        h._th_attack_active = True
        h._th_attack_progress = (i % COOLDOWN) / float(COOLDOWN)
        h._th_attack_phase = PHASE_ORDER[min(5, i // 6)]
        if i % 10 == 0:
...
def test_draw_thalgryn_lane_boss_dibawah_anggaran():
    h = fresh_hero()
    ...
    assert ms < 8.0, "draw_thalgryn(live) %.2f ms/frame" % ms
