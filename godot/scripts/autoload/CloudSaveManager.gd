...
func _python_json_string(value: String) -> String:
	var result := "\""
...
func _python_float_json(value: float) -> String:
...
func _decimal_exponent(value: float) -> int:
...
func _fixed_to_scientific(fixed: String) -> String:
	var sign := ""
	var unsigned := fixed
...
func _large_fixed_scientific(value: float, exponent: int,
...
func _scaled_scientific(value: float, exponent: int,
		significant_digits: int) -> String:
	var decade := pow(10.0, exponent)
	if decade == 0.0 or not is_finite(decade):
		return ""
...
func _scientific_from_digits(digits: String, exponent: int,
		sign: String) -> String:
...
func _dispatch_result(kind: String, status: Dictionary,
	...
	if kind == "download":
		_handle_download_ok(aux, callback, message)
		return
...
func _handle_download_ok(aux: Dictionary, callback: Callable,
		message: String) -> void:
