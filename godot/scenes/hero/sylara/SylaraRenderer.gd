...
const CAPE_SEGS: Array = [8.5, 8.5, 9.5]
const CAPE_W: Array = [5.5, 4.6, 3.2]
const HOOD_SEGS: Array = [5.5, 5.0]
const HAIR_SEGS: Array = [7.5, 6.5, 6.5]
...
func _draw_shadow(p, _j: Dictionary) -> void:
	var c := Vector2(p.root_x * 0.4, 0.0)
...
func _draw_cape(p, j: Dictionary) -> void:
	var anchor := j["chest"] + Vector2(-3.5, -2.5)
	var prev := anchor
	for i in 3:
		var seg_len: float = CAPE_SEGS[i]
		var seg_w: float = CAPE_W[i] + p.cape_flare * (0.4 + float(i) * 0.25)
...
func _draw_hair(p, j: Dictionary) -> void:
	var anchor := j["head_c"] + Vector2(-2.5, -1.0)
	var prev := anchor
	for i in 3:
...
func _draw_quiver(_p, j: Dictionary) -> void:
	var base := j["chest"] + Vector2(-5.5, 1.5)
...
func _draw_feet(_p, j: Dictionary) -> void:
	for ankle_key in ["ankle_b", "ankle_f"]:
		var is_front := ankle_key == "ankle_f"
		var toe_key := "toe_f" if is_front else "toe_b"
		var ankle := j[ankle_key] as Vector2
		var toe := j[toe_key] as Vector2
		var col := Pal.LEATHER if is_front else Pal.LEATHER_DARK
...
func _draw_tunic(p, j: Dictionary) -> void:
	...
	var flare := 2.2 + p.cape_flare * 3.2
	var poly := PackedVector2Array([
...
func _draw_torso(_p, j: Dictionary) -> void:
	var chest := j["chest"] as Vector2
...
func _draw_belt(_p, j: Dictionary) -> void:
	var hip := j["hip"] as Vector2
...
func _draw_vest(_p, j: Dictionary) -> void:
	var chest := j["chest"] as Vector2
...
func _draw_hood(p, j: Dictionary) -> void:
	...
	var anchor := head_c + Vector2(-3.2, -HEAD_R + 1.5)
	var prev := anchor
	for i in 2:
...
func _draw_head(p, j: Dictionary) -> void:
	...
	var eye_x := head_c.x + 2.6 + lean * 2.0
	var eye_y := head_c.y - 1.6
...
func _draw_bow(p, j: Dictionary) -> void:
	...
	var ctrl_upper := grip + bow_dir * BOW_LEN * 0.55 + bow_perp * 4.5
	...
	var ctrl_lower := grip - bow_dir * BOW_LEN * 0.55 - bow_perp * 4.5
	_draw_bow_limb(grip - bow_dir * 3.2, ctrl_lower, tip_lower, 3.2, 1.6)

	# Recurve horn tips
	var tip_upper_end := tip_upper - bow_perp * 3.2
	var tip_lower_end := tip_lower + bow_perp * 3.2
	...
	if p.bow_draw > 0.01:
		...
		var pull := bow_perp * (-p.bow_draw * 11.0)
		...
		if p.bow_draw > 0.25:
			...
			var arrow_tip := grip + bow_dir * (BOW_LEN + 8.0)
...
func _draw_rim(p, j: Dictionary) -> void:
	var head_c := j["head_c"] as Vector2
	var rim_col := Color(Pal.RIM.r, Pal.RIM.g, Pal.RIM.b, 0.4 * p.alpha)
	# Head & Hood rim
	draw_arc(head_c + Vector2(-1.2, -1.2), HEAD_R * 0.95, 2.7, 4.3, 6, rim_col, 1.2)
	# Shoulder rim
	draw_line(j["sh_f"] + Vector2(-1.2, -2.2), j["sh_f"] + Vector2(2.2, -3.2), rim_col, 1.2)
...
func _draw_wind_wisps(p, j: Dictionary) -> void:
	...
	var wisp_col := Color(Pal.WIND_LIGHT.r, Pal.WIND_LIGHT.g, Pal.WIND_LIGHT.b,
		0.55 * glow * p.alpha)
	for i in 3:
		var t := phase * 3.2 + float(i) * 2.1
		var off := Vector2(sin(t) * 4.5, cos(t * 1.4) * 3.5)
...
func _draw_hurt_flash(p, j: Dictionary) -> void:
	var chest := j["chest"] as Vector2
	var head_c := j["head_c"] as Vector2
	var col := Color(Pal.HURT_TINT.r, Pal.HURT_TINT.g, Pal.HURT_TINT.b, 0.3 * p.hurt_tint)
