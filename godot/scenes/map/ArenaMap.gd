...
func _draw_lane(pts: PackedVector2Array, d: Dictionary):
	...
	# anti-aliased join bulat — mencegah patah scher di belokan mid yang lama zig-zag
	draw_polyline(pts, d["path_border"], w + 10.0, true)
	draw_polyline(pts, d["path"], w, true)
	var lit: Color = d["path_light"]
	draw_polyline(pts, Color(lit.r, lit.g, lit.b, 0.45), w * 0.42, true)
	# cobblestone: titik terang berkala — jarang agar tidak berantakan (dari tiap 5 → 7)
	for i in range(0, pts.size(), 7):
		...
		for s in [-1, 1]:
			draw_circle(p + perp * s * LANE_HALF_WIDTH * 0.55, 2.2,
				Color(cobble.r, cobble.g, cobble.b, 0.4))
	# ── LUMUT + RETAKAN JALUR (path_moss / path_crack) ──
	# Dipersparse agar lane terlihat bersih: moss 22%→12%, crack 12%→8%, step 3→5.
	# Vulkanik crack (255,100,20) tetap menyala tapi tidak menutupi seluruh jalur.
	...
	for i in range(0, pts.size(), 5):
		...
		var off := perp * (float((variant % 7) - 3) / 3.0) * LANE_HALF_WIDTH * 0.6
		if variant < 12:
			draw_circle(p + off, 2.8, Color(moss.r, moss.g, moss.b, 0.38))
		elif variant < 20:
			draw_line(p + off - fwd * 4.5, p + off + fwd * 4.5,
				Color(crack.r, crack.g, crack.b, 0.6), 1.4)
