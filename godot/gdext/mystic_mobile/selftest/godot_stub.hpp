...
// Packed* untuk MENJALANKAN mobile_processor.cpp di luar engine (self-test).
//
// Meniru SEMANTIK Godot 4.3 yang dipakai kode generated (pola
// godot/gdext/mystic_ui/selftest/godot_stub.hpp, ditambah Rect2 karena
// MysticMobile mengembalikan geometri tombol HUD, plus CharString/utf8,
// get_type, operator Array/Dictionary, Dictionary::get, dan Array::get
// yang dipakai payload_summary cloud_save):
//
...
public:
    std::string data;
    // Godot CharString::get_data() — penunjuk ke buffer NUL-terminated.
...
public:
    ...
    // Godot String::utf8() — buffer UTF-8 (stub: std::string apa adanya).
...
public:
    ...
    // Godot Variant::operator double() (FLOAT -> f; INT -> (double)i).
    ...
    // Godot Variant::operator Dictionary()/Array(): tipe tak cocok ->
    // hasil kosong (core/variant/variant.cpp: tidak ada error).
...
public:
    ...
    // Godot Array::get(index) — dipakai payload_summary.
    Variant get(int64_t p_index) const { return items[(size_t)p_index]; }
...
public:
    ...
    // Godot Dictionary::get(key, default) — dipakai payload_summary
    // (cloud_save.get_payload_summary -> data.get("meta_gold", 0)).
    Variant get(const Variant &key, const Variant &p_default) const {
        return has_key_variant(key) ? Variant(get_const(key)) : p_default;
    }
};
...
inline Variant::operator Dictionary() const {
    return (type == DICTIONARY && dict) ? *dict : Dictionary();
}

inline Variant::operator Array() const {
    return (type == ARRAY && arr) ? *arr : Array();
}
...
// ── Kelas dasar + makro godot-cpp yang dipakai mobile_processor.h ──────────
//
