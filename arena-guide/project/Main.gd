...
func _ready() -> void:
	...
	print("[Main] Level %d: %s | gold awal %d | Tema map: %s." % [int(level_data["level"]), str(level_data["name"]), int(level_data["name"]), int(level_data["starting_gold"]), str(map_theme.get("name", "?"))])
