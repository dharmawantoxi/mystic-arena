...
func draw_on(host: CanvasItem) -> void:
	...
	var title: String = "ITEM FORGE — toko biru (H / klik ITEM)" if mode=="item" else "HERO SHRINE — toko merah (klik SHOP)"
...
func _draw_item_grid(host: CanvasItem, px: float, py: float, pw: float, ph: float, font: Font) -> void:
	...
	var twp: float = font.get_string_size("< PREV", HORIZONTAL_ALIGNMENT_LEFT, -1, 10).x
	var twn: float = font.get_string_size("NEXT >", HORIZONTAL_ALIGNMENT_LEFT, -1, 10).x
	...
	if _main != null and _main.hero != null:
		...
		var sx2: float = px + pw*0.5 - 180
		for i in 6:
			var r: Rect2 = Rect2(sx2 + float(i)*54, inv_y+8, 48, 32)
			_inv_rects[i] = r
